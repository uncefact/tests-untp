...
- **Update (2026-09-03):** A claim now records the library record its request produced rather than the `Credential` row. The column is `recordId` and its foreign key points at `LibraryRecord` (ADR-053), so one column serves a native issuance and an external registration alike, and deleting the record still frees the key by cascade. The decisions are unchanged.
