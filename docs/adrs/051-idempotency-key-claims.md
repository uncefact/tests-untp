...
- **Status:** accepted
- **Update (2026-09-17):** Batch submission (ADR-060) takes its own `Idempotency-Key` on the `CredentialBatch` row rather than through the `IdempotencyKey` table, because a batch is not a library record and must remain readable as an expiry tombstone. The rule recorded here still governs credential issuance and external registration.
- **Update (2026-09-07):** Registering a credential received from a third party also refuses content the tenant already holds, answering `DUPLICATE_CREDENTIAL` (#956). That check is on the signed credential's own bytes and runs alongside the key, so it is not the payload-digest mechanism rejected below as a replacement for the header. The claim is taken first as decision 1 requires, and the duplicate rejection releases it, so the key stays free for a fresh registration once the duplicate is resolved. Issuance is unchanged.
...
- **Update (2026-09-03):** A claim now records the library record its request produced rather than the `Credential` row. The column is `recordId` and its foreign key points at `LibraryRecord` (ADR-053), so one column serves a native issuance and an external registration alike, and deleting the record still frees the key by cascade. Because one column serves both, the link is conditional on the claim's operation as well as on its still-null `recordId`, so a record can only be attached to a claim of its own operation (decision 8), and an offer of the wrong one fails as the caller defect it is rather than as the lost-claim `409`. The decisions are unchanged.
