...
- **Date:** 2026-07-15
- **Status:** accepted
- **Update (2026-09-17):** Conformity Schemes now run four validation steps, with Structural Parse between Schema Validation and JSON-LD context validation; the additional step remains part of the per-instance result and report readiness model.
- **Update (2026-09-17):** validation steps may carry additive typed artefact-failure metadata used by cards and reports; the shared per-instance model and report-readiness rules remain unchanged.
- **Update (2026-09-08):** amends the Decision's report-readiness rule. For the downloadable report (#814) a loaded link set is a reportable family whose stored result, the Schema Validation step, must be terminal; its derived Link Type Coverage step (#1007) may be recorded as pending and does not change the link set's status, while a coverage failure fails the link set and the report. Still-encrypted credential instances are excluded from the report and do not hold generation (#813). A generated report is invalidated when link set instances, URL bindings, credential instances or scheme instances change.
