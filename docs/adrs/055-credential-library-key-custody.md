...
- **Date:** 2026-08-31
- **Status:** accepted
- **Update (2026-09-11):** The coverage listed under decision 7 is unchanged, and the summary derived from it now requires a passing `proof`: a generation the verifier refused on the validity window is asked again with the window skipped so proof and status are established, and `temporal` is also judged from the credential's own validity claims. See ADR-053's update of the same date.
- **Update (2026-09-09):** ADR-057 adds a fourth custody state to decision 5's detail response: a key is held (`hasKey: true`) but cannot be returned (`decryptionKey: null`), conveyed by a `DECRYPTION_KEY_UNAVAILABLE` warning on the record, instead of the sanitised 500 the detail route answered before. Re-verification is unchanged: a stored key that cannot be unwrapped still settles the generation as `STORED_COPY_UNAVAILABLE` with `retryable: true`, and what changes is that the detail poll now shows that state rather than failing. Key material still appears on the detail route and nowhere else.
...
- **Update (2026-09-07):** Re-verification of a protected copy reads the pinned copy on the worker and checks the supplier source separately in the request. A copy that cannot be read back as the document that was stored settles the generation as `STORED_COPY_UNAVAILABLE`, and a copy that reads back but fails its digest check settles it as `STORED_COPY_CORRUPT`, both with `retryable: false`, but it does not clear or change any custody column. The detail route continues to report `hasKey` from the key held by this service. A stored key that cannot be unwrapped under the active deployment encryption key is currently mapped to the same failure code with `retryable: true`, as an interim rule shared with #769. See [Re-verify a library record](../../documentation/docs/reference-implementation/api/library.md#re-verify-a-library-record).
- **Update (2026-09-09):** #957 does not land a late-key writer, and re-verification still takes no body. A record that already holds a **stored** unopened copy (`storageUri` set, `decryptionKey` null) is still refused synchronously with `DecryptionRequiredError`, no fetch and no generation, because this service never stored the supplier key that would open that copy. A record with no durable copy is not refused. It is reserved and fetched like any other, because the flag saying it was encrypted describes what was last observed rather than what the source will return now. A re-fetch that returns unopenable ciphertext settles a `202` generation carrying `DECRYPTION_REQUIRED`, storing the ciphertext fresh when the row held no content identity and refusing to replace one it already held. Accepting a caller-supplied key against either kind of record remains tracked separately, by #958.
- **Update (2026-09-09):** #958 adds the late-key writer. The key-bearing form runs in the request, reads an unopened durable copy without fetching its supplier source, verifies the raw storage digest before opening it, and atomically replaces that custody with a receiver-protected copy before enqueueing reference-only verification. A record without a durable copy fetches its source with the request-local key. Wrong-key and terminal acquisition failures retain the raw custody and never upload a replacement. A key is still never persisted, logged, queued or returned, and an interrupted attempt requires the caller to send it again.
- **Update (2026-09-10):** Worker-settled verification now executes the advisory `schemaConformance` check against the system core schema for the record's stored core type and version, followed by JSON-LD expansion. It does not check extension schemas, never changes the verification summary, and records the first violation as a `SCHEMA_CONFORMANCE_ADVISORY` warning when the result is `fail`.
