...
- **Update (2026-09-10):** Worker-settled verification now executes the advisory `schemaConformance` check against the system core schema for the credential's declared type and version, followed by JSON-LD expansion. It does not check extension schemas, never changes the verification summary, and records the first violation as a `SCHEMA_CONFORMANCE_ADVISORY` warning when the result is `fail`.
