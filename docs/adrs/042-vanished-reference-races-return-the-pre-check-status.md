...
2. **The rule covers references the caller supplied.** A dependency the server selected on the caller's behalf (a resolved default service instance, a matched entity) is not a caller reference, so its race is a failed server-side selection and follows ADR-036's sanitised path rather than returning a 404 that names an id the caller never sent.
3. **Multi-key writes match the violated column first.** Where the write carries more than one foreign key, `isForeignKeyViolationOn` matches the violated column before the pre-check's error is thrown, so a violation on any other column (for example a tenant deleted mid-request) rethrows into ADR-036's sanitised backstop instead of carrying a misattributed message.
4. **Applied at:** identifier creation and scheme-qualifier recreation (scheme), data-model creation (parent config), DID creation and import (caller-supplied service instance, previously the helper's 400; a server-resolved instance's race stays on the sanitised path), registrar creation and update (IDR service instance, previously a 400), identifier-scheme creation and update (registrar and IDR service instance, previously a disjunctive 400), render-template creation (data model, previously matched column-blind), and link-registration bulk creation (identifier).

## Consequences

- Easier: the race maps to a status every affected route already documents, so no new response codes appear anywhere. The one description that had documented the race under the old mapping (the DID import route's 400) was corrected to match.
...
- Harder: the repository catch must repeat the pre-check's message string. For some sites the pre-check sits in the same function or file; for others (DID, render-template, link-registration) it lives in the route or a wrapper, where the duplicated string is likelier to drift and relies on the paired tests to catch it.
