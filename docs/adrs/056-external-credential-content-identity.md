...
- **Update (2026-09-09):** DELETE /api/v1/library/{id} calls `promoteExternalCredentialDigest` under the complete ordered parent lock set in its transaction before cascading the deleted external family. A holder's oldest advisory becomes the digest holder and the remaining advisories are repointed at it. The deleted holder's stored copy is the only copy eligible for best-effort cleanup.
