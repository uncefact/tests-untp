...
- **Date:** 2026-05-21
- **Status:** accepted
- **Update (2026-09-17):** ADR-059 supersedes the section 3 statement that validation does not check score values: score codes are now checked for membership in the frameworks the scheme publishes, and scheme and profile references that name the wrong catalogue tier are diagnosed. The `validateConformityClaim(claim, scheme)` signature in section 4's function table now takes an optional third argument, a `ConformityReferenceResolution` carrying the caller's catalogue lookups, which the tier diagnosis needs. The two-argument call is unchanged. The other decisions in this ADR stand.
- **Update (2026-09-01):** ADR-054 supersedes this ADR's execution model: the in-process/no-worker decision, the instrumentation-hook refresh interval, the no-new-containers consequence, the accepted per-replica timer limitation, and the rejection of a separate worker. Background work, including the scheme refresh, moves to a Postgres-backed job queue consumed by a dedicated worker container (#984, #985). The CVC domain decisions in this ADR stand. The body retains the original framing for historical record.
...
- **Update (2026-08-13):** Seeded (`SYSTEM_SEED`) conformity schemes now refresh their content from source, superseding two §1 sentences: "Operator-seeded schemes refresh on boot from local files only" and the restriction of periodic refresh to UNTP-discovered schemes. At boot the seed loader re-ingests every manifest entry (URL entries through the HTTP conditional-fetch chain; file entries through a local body-digest compare added to `resolveAndParseConformityScheme`, so unchanged bytes skip parse and persist). Between boots an in-process interval, registered in the Next.js node startup hook per §6's topology, re-fetches URL-seeded rows from the database at the `CVC_REFRESH_INTERVAL_HOURS` cadence (default 24h, with jitter per §1) independent of `CVC_REGISTRY_URL`, so seed-only deployments pick up publisher updates without a reboot; rows seeded from files refresh at boot only, and a new nullable `seedEntryKind` column records which is which. The ingest persist, seeded-scheme eviction, and the orphan-criterion sweep now serialise on a per-tenant Postgres advisory lock, a concurrency boundary the §1 partial-failure model left implicit. The UNTP discovery trigger (#690) and manual refresh endpoint (§8) remain separate, unimplemented follow-ups. See #728.
- **Update (2026-08-12):** The seed-loader collision policy for manifest MEMBERSHIP changes from insert-only-if-absent (recorded in point 1 of the 2026-05-28 update, implemented in #689) to a full manifest reconcile; this update supersedes that point for membership. Scheme CONTENT fetching is unchanged: an existing conformity scheme row's content is still never re-fetched or overwritten by the seed (that changes with #728). The custom seed manifest is the source of truth for the rows it owns: a manifest entry with no matching database row is inserted, a changed entry is updated in place (including reparenting an identifier scheme or qualifier), and a database row whose manifest entry has been removed is deleted. Ownership is recorded per row in a `source` enum (`RecordSource`: `CORE_SEED` for rows the core seed creates, `CUSTOM_SEED` for manifest-managed rows, `USER` for API/UI-created rows, the default for pre-existing rows) on `Registrar`, `IdentifierScheme`, `SchemeQualifier`, `DataModel`, and `RenderTemplate`; conformity schemes keep their existing `source` enum, with `SYSTEM_SEED` rows reconciled by source URL against the manifest. The reconcile only ever deletes `CUSTOM_SEED` (or, for conformity schemes, `SYSTEM_SEED`) rows, and a deletion whose cascade would reach rows the manifest does not own (any non-manifest identifier scheme, qualifier, render template, or data-model extension under a removed parent; registered identifiers) fails the seed with a named error instead of proceeding. A manifest section key that is absent leaves that entity type unmanaged for the boot; a key explicitly set to an empty array removes all manifest-owned rows of that type. See #727.
- **Update (2026-08-16):** Where the provenance of a projected claim value lives, recorded when advisory warning pointers were made resolvable against the submitted credential. `ConformityClaim` stays semantic: its fields are the conformity capabilities `validateConformityClaim` runs rules against, and a path into one input document is not one of those, so the extractor records provenance *beside* the claim rather than on it. A version that records provenance returns the claim together with a map from claim pointers to paths in the `credentialSubject` it extracted from, and the consumer substitutes those before returning warnings. This is not a case the "new field is the sanctioned path" rule of the 2026-07-13 update covers: that rule governs new spec capabilities, and using it to hang extractor bookkeeping on an interlingua exported from `@uncefact/untp-utils` would make a document-neutral type carry document shape. The map is recorded per addressable value rather than per parent, because a suffix appended to a parent's path can name the wrong value: topic entries without a usable `id` are dropped while extracting, so a projected topic index need not match its source index. Values the validator can point at but the document does not contain (an unspecified profile, a criterion the claim never declared) have no entry, and a consumer omits the pointer for those rather than returning one relative to a projection its caller cannot obtain. See #753.
- **Update (2026-08-16):** Naming decision for the RI's API surface. The canonical name is **Conformity Vocabulary Catalogue**: the endpoints expose the catalogue of conformity schemes held by the RI, so they take the catalogue name, with `CVC` as its abbreviation, retained as the `/api/v1/cvc/*` path segment that shipped in the 0.3.0 release. The Swagger surface uses one tag named `Conformity Vocabulary Catalogue` across the declaration and the three browse routes, replacing the previously declared-but-unused `CVC` tag and the ad-hoc `Conformity Vocabulary` operation tag. The `@uncefact/untp-utils/conformity-vocabulary` sub-entry keeps its name: it operates on the vocabulary items (schemes, profiles, criteria) rather than the catalogue. See #729.
- **Update (2026-08-17):** The three `:id` detail routes (`GET /api/v1/cvc/schemes/:id`, `GET /api/v1/cvc/profiles/:id`, `GET /api/v1/cvc/criteria/:id`) and the tenant-import route described in §2 (`POST /api/v1/cvc/schemes`) are not implemented. The RI currently exposes only the three list routes: `GET /api/v1/cvc/schemes`, `GET /api/v1/cvc/profiles`, and `GET /api/v1/cvc/criteria`.
- **Update (2026-09-07):** The Playground now expands a credential's JSON-LD server-side through the same guarded document loader §7 prescribes (`POST /api/context`, over `expandJsonLd` in `@uncefact/untp-utils`), where it previously expanded in the browser through jsonld.js's default loader; remote `@context` fetches from the Playground therefore pass `validatePublicUrl`, are https-only, and use the bundled contexts when a host is down. `describeJsonLdFailure` is the one classifier for JSON-LD failures across the RI and the Playground and now separates three classes, a context that could not be fetched (`context-fetch`), one that was fetched but is not usable as a context (`context-invalid`, including a body that is not JSON), and a document that fails against its contexts (`document`); the RI reports the first two under `JSONLD_CONTEXT_FETCH_FAILED`, whose contract already reads "fetched or used". jsonld.js syntax-error messages are never echoed to a caller because some interpolate values from the caller's document; the recognised code and an allowlist of identifiers are the diagnostic. See #828 and #1012.
- **Update (2026-09-07):** The JSON Schemas and JSON-LD contexts UNTP publishes, from 0.6.0 onwards, now ship inside `@uncefact/untp-utils` and stand in when the publishing host cannot deliver one, so a host outage no longer marks a scheme `JSONLD_EXPANSION_FAILED` (§1) for a context the bundle carries, nor fails the `ConformityScheme.json` fetch §7's validation gate depends on. §7's SSRF position is unchanged and deliberately so: a URL `validatePublicUrl` refuses is never covered by the bundle, because a UNTP host resolving to a private address is a signal the operator must see. Scheme documents themselves are fetched from their owners as before and are not bundled. `BUNDLED_ARTEFACTS_FALLBACK=false` restores the previous behaviour on both the CVC and issuance paths. See #1006 and [Bundled UNTP Artefacts](https://github.com/uncefact/tests-untp/blob/next/documentation/docs/reference-implementation/operations/bundled-untp-artefacts.md).
...
| `CVC_REFRESH_INTERVAL_HOURS` | `24` | Periodic refresh cadence. Since the 2026-08-13 update this also drives the seeded-scheme refresh; maximum 500 hours. |
...
| `GET` | `/api/v1/cvc/schemes/:id` | Not implemented. Single scheme with its profiles and criteria. |
| `GET` | `/api/v1/cvc/profiles/:id` | Not implemented. Single profile with its criteria. |
| `GET` | `/api/v1/cvc/criteria/:id` | Not implemented. Single criterion. |
| `POST` | `/api/v1/cvc/schemes` | Not implemented. Tenant import. Body `{ url, cvcSpecVersion? }`. Returns `409 Conflict` when the URI is already present in the system-tenant lane. |
...
| `@uncefact/untp-utils/loaders` | `createSchemaLoader` (cached) | Fetches and caches JSON Schemas (the UNECE-published `ConformityScheme.json`, etc.). |
