...
- **Update (2026-09-11):** the worker's recorded boot contract is now reached after a shared configuration preflight that runs first in both processes.
- **Update (2026-09-05):** #985 landed decision 1's worker container and entrypoint, and the boot and stop contract it needed is recorded on the Startup operations page (`documentation/docs/reference-implementation/operations/startup.md`, "Worker Boot"). Four facts about it are not derivable from the decisions above. The worker runs no migrations, backfills or seed, and instead checks at boot that every migration its own build ships is already applied, so a database ahead of the worker passes and a rolling deploy's older worker still starts. It refuses to boot without `DATA_ENCRYPTION_KEY`, which decision 6 does not cover because that key belongs to the deployment rather than to a caller. It serves no HTTP, so it proves itself by publishing a heartbeat file whose age the container health check reads, and an unhealthy worker is a signal for an orchestrator rather than a reason for the process to exit. Its stop is bounded by a job drain inside a process deadline inside the container's grace period, so the runtime never kills a worker that is still shutting down on its own terms. The version-skew window named under Consequences also gains a queue-side rule, because a worker strips payload keys it does not recognise. A new payload field may only be observability and must be optional, and anything with business effect is a new queue name instead, a rule the type guard in `packages/reference-implementation/src/worker/payload-contract.test.ts` holds each payload to. Decision 8, moving scheduled work onto the queue, is not executed. The conformity-scheme refresh still runs on the in-process interval in the web boot, and moving it onto the queue is outstanding.
- **Update (2026-09-07):** The library's pending verification generations now have a worker-owned reconciliation job. It runs every ten minutes, selects up to 500 rows per tick whose enqueue marker has outlived the verification retry ladder, or is missing on a row that is itself older than that bound, and settles them as retryable `VERIFICATION_UNAVAILABLE` failures after a policy bound of at least 30 minutes. It does not re-enqueue the original job, so the caller re-verifies to create the next generation. The state guard means a late worker attempt cannot overwrite the reconciliation result. See [Pending Verification Reconciliation](../../documentation/docs/reference-implementation/operations/startup.md#pending-verification-reconciliation).
- **Update (2026-09-09):** A no-copy library-record recovery (#957) reserves generation N+1 as `PENDING` with no job before it fetches the source, outside any transaction, then finalises that exact reservation in a second transaction once the fetch, and any store, completes. Decision 4's rule that the job is enqueued in the same transaction as the state it will settle still holds for every branch that enqueues. What no longer holds is describing a failure after this reservation, whether in the fetch, the store, or the encryption preflight the finalisation can now reach, as leaving no generation behind. The reservation itself is a generation, settled `FAILED` rather than left uncreated. This is enforced in code. A throw from the fetch or the finalisation is caught around the reservation and settles it `FAILED` and retryable before the caller's error propagates, classified from the cause. An encryption or storage-resolution failure names itself, a finalisation that collided twice on the same content identity is reported as a collision, and anything else is reported generically with the run id logged. Nothing therefore reaches only the reconciliation sweep except a request that genuinely never returns. Two of those settlements answer `202` with the settled generation rather than an error, because the reservation the caller asked for demonstrably exists and is demonstrably finished with. They are a job queue that cannot be started after the reservation exists, and a finalisation whose lock set kept moving past its bounded restarts. Both answer `202` only once the settle write itself has committed. If that settle write also fails, which is a transient database fault separate from whatever triggered it, the reservation stays `PENDING` and the request answers a sanitised `500` rather than a false promise of a settled generation. A reservation whose fetch never returns, through a crash or a killed process, is exactly the abandoned-run case this ADR's 2026-09-07 update already covers. The reconciliation sweep settles it once its `lastEnqueuedAt` and `requestedAt` cross the abandonment bound, and that sweep's settlement now also rechecks the abandonment predicate inside its own `UPDATE`, so a reservation finalised between the sweep's selection and its write is not overwritten by a stale decision. The sweep remains the eventual backstop for any reservation neither path could settle.
- **Update (2026-09-09):** #958 extends the reservation contract for key-bearing recovery. A request carrying a supplier key is refused before any reservation when the record is native or its durable copy is already receiver-protected, and it skips the early pending join. The locked reservation then answers `conflict` for an eligible record whose newest generation is still pending, and `not-applicable` for a record that became receiver-protected between the caller's read and the lock. An eligible record whose generation advanced in that window reserves the NEXT generation from the locked state rather than answering with the superseding one, so a supplied key is never acknowledged without being consumed. The same transaction supplies the current custody snapshot, including key presence and `encrypted`, from which the acquisition mode and the finalisation fence are chosen. This keeps the key-bearing path inside the request, while the replacement and the reference-only worker job remain committed atomically with the settled reservation state.
- **Update (2026-09-10):** Every queued job, including the pending-verification reconciliation sweep, now receives its expiry from `WORKER_JOB_TIMEOUT_SECONDS` (default 300 seconds). The worker budgets each verification attempt from the job's expiry, and its heartbeat permits an active job the same budget plus its health margin. The reconciliation cutoff is therefore 60 minutes at the default rather than 30 minutes.
- **Update (2026-09-11):** The job envelope carries an optional `correlationId`, and the worker wraps each job in its own span. The optional field exercises the version-skew rule for observability-only payload additions.
- **Update (2026-09-17):** Batch issuance (#663) now runs on this substrate, and its job-resource shape is recorded in ADR-060. It adds three queue names, `credentials.issue-batch`, `credentials.reconcile-batches` and `credentials.expire-batches`, and its payload carries only `{ batchId, tenantId }`, so status is read from the batch rows and never from the queue. See the [batch issuance runbook](../../documentation/docs/reference-implementation/operations/batch-issuance.md).
...
7. **The application reconciles work the queue loses; the dead-letter queue is telemetry, never correctness.**
...
8. **Scheduled work moves onto the queue.**
