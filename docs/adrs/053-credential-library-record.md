...
- **Date:** 2026-08-25
- **Status:** accepted
- **Update (2026-09-09):** ADR-057 changes what the collection routes do when a row breaks this read model: `GET /library` and `POST /library/batch-get` report that row by id in a `failures` array and return the rest, instead of failing the whole response. The read model and its invariants are unchanged; the detail route still answers a whole-request error for a record it cannot build.
- **Update (2026-09-09):** GET /api/v1/library uses the extracted core credential type as its authority and falls back to an external record's declared type only while that extracted value is null. The registered-data-model fallback remains explicitly deferred from #962 and is owned by this epic's next ticket that touches extraction, or #965 if no such ticket is assigned. #962 adds no index. The tenant-led effective-date expression index remains the first candidate and needs measurement against the real list query. The parent and child record decision is unchanged.
- **Update (2026-09-09):** Recipient annotation edits advance `ExternalCredential.annotationVersion` and touch the parent `LibraryRecord.updatedAt` in one Read-Committed transaction. The transaction takes the parent `FOR UPDATE` lock before reading the child, comparing the token or writing, so annotation edits serialise with cascading deletion and never alter extracted fields or verification runs.
- **Update (2026-09-09):** DELETE /api/v1/library/{id} takes one ordered tenant-scoped `FOR UPDATE` lock over the deleted record and every parent its promotion will touch (planned by an unlocked read and re-planned once if an advisory attaches before the lock), using the same lock discipline as recovery, before reading the record. It refuses only a native record in that tenant, and deletes an external parent in one Read-Committed transaction. The parent cascade removes its child, verification generations and idempotency claim. Durable-copy cleanup runs after commit from the row's recorded coordinates and is best-effort.
...
- **Update (2026-09-07):** Decision 4's equivalence from generation 2 onward holds for what runs, not for what is published. Re-verification (#957) runs a native record's second and later generations through the same pipeline as an external record's, and the worker's real results are stored on the run, but every response projects a native record's `retrieval`, `decryption` and `digest` as `not_run` at every generation, because those checks describe acquiring a credential from another party and a native copy was never acquired. Decision 7's envelope schema gains an optional `sourceChanged` and `lastSourceCheckAt` pair, published together on a settled external generation that compared the supplier source against the copy's recorded source digest, and a `complete` summary is derived from the checks as published rather than as stored. See [Re-verify a library record](../../documentation/docs/reference-implementation/api/library.md#re-verify-a-library-record).
