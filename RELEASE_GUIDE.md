...
- [ ] All Docker images build from a cold cache: `docker compose build --pull --no-cache` completes for every service that declares a `build:` section. Cached layers and change-triggered CI cannot surface drift in floating build inputs (base image tags, toolchain defaults), so a fresh build is the only check that exercises them.
