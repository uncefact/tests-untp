...
- **Private URL fetch opt-in.** For local development only, the off-by-default `FETCH_ALLOW_PRIVATE_URLS=true` setting lets any browser user make `/api/fetch` retrieve HTTP or HTTPS documents from private, loopback and other reserved destinations, including cloud metadata addresses, directly or through redirects. ([#1090](https://github.com/uncefact/tests-untp/pull/1090))
...
- **One failure presentation across the tabs.** Credential steps, conformity scheme steps and the link set Schema Validation step open the same drawer of issue and warning cards, with a card built from the step failure when no error list exists; the failure class text no longer appears on step rows or as a card above the list, the drawer identifies a skipped scheme step as `Not executed`, the remove control of a card stays within its header row, link-row transient state lives in the action slot and settled status sits beside the label, settled link outcomes open the matching Credentials card, clicking Verify or Resolve on a link set row no longer changes the row's size, and the HTML report shows the same issue cards as the drawer. ([#1090](https://github.com/uncefact/tests-untp/pull/1090))
