...
A check that could not run is still shown as a failure, with a line saying why. A scheme whose version has no parser in the Playground says so. A scheme published before UNTP 0.7.0 keeps its single schema-selection diagnosis, and the structural check records that schema selection prevented it from running. When version detection fails, the three later checks all report that they were skipped. Schema Validation and the JSON-LD check still run independently of each other, so a scheme with a blank `id` is reported by both the structural check and the JSON-LD check.
