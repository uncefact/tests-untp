...
export interface ReportMeta {
  /** Version of the app under test (untp-playground package.json, overridable for deployed targets). */
  appVersion: string;
...
/**
 * Versions of the dependent services actually running for this suite (VCKit, Postgres, ...),
 * read from the live Docker Compose stack rather than re-parsing docker-compose.e2e.yml's
 * declared tags — so a locally overridden image tag is still reported correctly. Only queried
 * when baseUrl is local: for a deployed-instance run there is no local stack to inspect, and
 * querying it anyway risks reporting unrelated containers left running from other work.
 */
