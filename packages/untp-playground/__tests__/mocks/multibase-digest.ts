/**
 * Deterministic test stub for `@uncefact/untp-utils`'s `MultibaseDigest`.
 *
 * Wired via `moduleNameMapper` in `jest.config.js`. The real class ships as
 * ESM and imports from `multiformats` via subpath exports that only declare
 * an `import` condition, which this package's Jest CJS resolver cannot
 * unpack without a bigger toolchain change. Production code consumes the
 * real package; only tests see this stub. Round-trip multibase encoding
 * coverage lives in `@uncefact/untp-utils` itself.
 *
 * Mirrors the equivalent stubs in `packages/services` and
