/**
 * @jest-environment jsdom
 */

import { Header } from '@/components/Header';
import { render, screen } from '@testing-library/react';
import { testSuiteVersion } from '../../config';
import { RuntimeConfigProvider, defaultConfig } from '@/contexts/RuntimeConfigContext';

const renderHeader = (configOverrides = {}) => {
  const config = { ...defaultConfig, ...configOverrides };
  return render(
    <RuntimeConfigProvider config={config}>
      <Header />
    </RuntimeConfigProvider>,
  );
};

describe('Header', () => {
  it('renders correctly', () => {
    renderHeader();
    const header = screen.getByRole('banner');
    expect(header).toHaveClass('border-b');
  });

  it('has correct container classes', () => {
    renderHeader();
    const container = screen.getByRole('banner').firstElementChild;
    expect(container).toHaveClass('container', 'mx-auto', 'p-8', 'max-w-7xl', 'flex', 'items-center', 'gap-4');
  });

  it('renders the default title and version badge', () => {
    renderHeader();
    const heading = screen.getByRole('heading', { level: 1 });
    expect(heading).toHaveTextContent('UNTP Playground');
    expect(heading).toHaveClass('text-2xl', 'font-bold');
    expect(screen.getByText(`v${testSuiteVersion}`)).toBeInTheDocument();
  });

  it('renders a custom title from config', () => {
    renderHeader({ headerTitle: 'My Custom App' });
    const heading = screen.getByRole('heading', { level: 1 });
    expect(heading).toHaveTextContent('My Custom App');
  });
});
