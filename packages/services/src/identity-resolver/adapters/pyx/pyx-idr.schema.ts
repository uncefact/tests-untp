...
export const pyxIdrConfigSchema = z.object({
  ...
  apiVersion: z.enum(['4.0']).default('4.0').describe('API Version||The Pyx IDR API version to use.'),
  defaultLinkType: z
  ...
  defaultIanaLanguage: z
    .string()
    .min(1)
    .describe(
      'Default Language||BCP 47 language tag wrapped into each variant\'s `hreflang[]` when the caller does not supply one (e.g., "en")',
    ),
