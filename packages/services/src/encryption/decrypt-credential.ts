...
const defaultLogger = createLogger().child({ module: 'decrypt-credential' });
 ...
 */
export const decryptCredential = (params: DecryptionParams, logger: LoggerService = defaultLogger): string =>
  Buffer.from(decryptCredentialToBytes(params, logger)).toString('utf8');
 ...
 */
export const decryptCredentialToBytes = (
  { cipherText, key, iv, tag, type }: DecryptionParams,
