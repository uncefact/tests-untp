...
describe('didWebToUrl', () => {
  ...
  it('handles localhost with percent-encoded port and path segments', () => {
    expect(didWebToUrl('did:web:localhost%3A3000:org:abc')).toBe(
      'https://localhost:3000/org/abc/did.json',
    );
  });

  it('converts a did:webvh domain to a well-known did.jsonl URL', () => {
    expect(didWebToUrl('did:webvh:example.com')).toBe(
      'https://example.com/.well-known/did.jsonl',
    );
  });

  it('converts a did:webvh with path segments to did.jsonl', () => {
    expect(didWebToUrl('did:webvh:example.com:org:abc')).toBe(
      'https://example.com/org/abc/did.jsonl',
    );
  });

  it('throws for unsupported DID method', () => {
    expect(() => didWebToUrl('did:key:z6Mk...')).toThrow(
      'Unsupported DID method for URL conversion',
    );
  });
});
...
describe('parseDidMethod', () => {
  ...
  it('throws for an invalid format', () => {
    expect(() => parseDidMethod('not-a-did')).toThrow(
      'Invalid DID string: not-a-did',
    );
  });

  it('throws when the method-specific-id is missing', () => {
    expect(() => parseDidMethod('did:web')).toThrow(
      'Invalid DID string: did:web',
    );
  });
});
