...
describe('cryptoService', () => {
  describe('decryptCredential', () => {
    const mockEncrypt = (data: string, key: Buffer, iv: Buffer): { cipherText: string; tag: string } => {
    ...
    it('should successfully decrypt a valid encrypted credential', () => {
      ...
      const { cipherText, tag } = mockEncrypt(plaintext, key, iv);
  ...
  describe('computeHash', () => {
    ...
    it('should use SHA256 as the default algorithm', () => {
      ...
      const manualHash = crypto.createHash(HashAlgorithm.SHA256)
