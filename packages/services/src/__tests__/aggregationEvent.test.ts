...
import { constructVerifyURL, constructIdentifierString } from '../utils/helpers';
...
jest.mock('../utils/helpers', () => ({
  ...
  constructIdentifierString: jest.fn(),
  ...
  it('should throw error if identifier not found', async () => {
    try {
      ...
      jest
        .spyOn(validateContext, 'validateTraceabilityEventContext')
        .mockReturnValueOnce({ ok: true, value: context } as unknown as Result<ITraceabilityEventContext>);
      (constructIdentifierString as jest.Mock).mockImplementationOnce(() => new Error('Identifier not found'));
