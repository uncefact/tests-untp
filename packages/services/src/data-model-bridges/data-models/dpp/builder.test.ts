...
describe.each(versions)('buildDppSubject (%s)', (_version, spec) => {
  ...
  describe('credentialSubject root', () => {
    ...
    it('sets granularityLevel to lowercased product.level when present', () => {
      const subject = bridge.buildSubject(createResolvedEntities({ product: createProduct({ level: 'BATCH' }) }));
      expect(subject.granularityLevel).toBe('batch');
    });

    it('sets granularityLevel to model when level is MODEL', () => {
      const subject = bridge.buildSubject(createResolvedEntities({ product: createProduct({ level: 'MODEL' }) }));
      expect(subject.granularityLevel).toBe('model');
    });

    it('sets granularityLevel to item when level is ITEM', () => {
      const subject = bridge.buildSubject(createResolvedEntities({ product: createProduct({ level: 'ITEM' }) }));
      expect(subject.granularityLevel).toBe('item');
    });

    it('omits granularityLevel when product.level is absent', () => {
      const subject = bridge.buildSubject(createResolvedEntities({ product: createProduct({ level: undefined }) }));
  ...
  describe('product', () => {
    ...
    it('maps product id and name', () => {
      const subject = bridge.buildSubject(
        createResolvedEntities({ product: createProduct({ id: 'did:web:example.com:product:1', name: 'My Product' }) }),
      );
      const product = subject.product as Record<string, unknown>;
    ...
    it('includes description when present', () => {
      const subject = bridge.buildSubject(
        createResolvedEntities({ product: createProduct({ description: 'A fine product' }) }),
      );
      const product = subject.product as Record<string, unknown>;
    ...
    it('omits description when absent', () => {
      const subject = bridge.buildSubject(
        createResolvedEntities({ product: createProduct({ description: undefined }) }),
      );
      const product = subject.product as Record<string, unknown>;
    ...
    it('omits registeredId and idScheme when primaryIdentifier is absent', () => {
      const subject = bridge.buildSubject(
        createResolvedEntities({ product: createProduct({ primaryIdentifier: null }) }),
      );
      const product = subject.product as Record<string, unknown>;
    ...
    it('includes batchNumber when present', () => {
      const subject = bridge.buildSubject(
        createResolvedEntities({ product: createProduct({ batchNumber: 'BATCH-2024-001' }) }),
      );
      const product = subject.product as Record<string, unknown>;
    ...
    it('omits batchNumber when absent', () => {
      const subject = bridge.buildSubject(
        createResolvedEntities({ product: createProduct({ batchNumber: undefined }) }),
      );
      const product = subject.product as Record<string, unknown>;
    ...
    it('includes serialNumber when present', () => {
      const subject = bridge.buildSubject(
        createResolvedEntities({ product: createProduct({ serialNumber: 'SN-12345' }) }),
      );
      const product = subject.product as Record<string, unknown>;
    ...
    it('omits serialNumber when absent', () => {
      const subject = bridge.buildSubject(
        createResolvedEntities({ product: createProduct({ serialNumber: undefined }) }),
      );
      const product = subject.product as Record<string, unknown>;
