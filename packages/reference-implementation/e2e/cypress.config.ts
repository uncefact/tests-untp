...
const baseUrl = process.env.CYPRESS_BASE_URL || 'http://localhost:3003';
...
// Versions of the dependent services (VCKit, storage service, identity resolver, Keycloak,
// Postgres, ...) actually running for this suite, plus the runtime that ran it — surfaced in
// the browser via Cypress.env(...) (see cypress/e2e/00-report-metadata.cy.ts) and written into
// run-info.json alongside reportMeta.
...
export default defineConfig({
  ...
  reporterOptions: {
    ...
    overwrite: false,
    charts: true,
    reportPageTitle: `UNTP Reference Implementation — E2E Report — app v${reportMeta.appVersion} — ${
      reportMeta.gitSha
    }${reportMeta.officialRun ? '' : ' (unofficial)'} — ${reportMeta.generatedAt}`,
  ...
  e2e: {
    ...
    setupNodeEvents(on) {
      // Report generation: purge previous report output, then generate the HTML report
      on('before:run', beforeRunHook);

      // Clean up all test artefacts after all specs complete, then generate the mochawesome report
      on('after:run', async (results) => {
        ...
        await finalizeReport(reportsRoot, reportMeta, {
          tenantMode: process.env.E2E_TENANT_MODE || 'open',
          baseUrl,
