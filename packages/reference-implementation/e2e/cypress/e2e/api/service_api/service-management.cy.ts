...
describe('Service API', { testIsolation: false }, () => {
  ...
  describe('CRUD operations', () => {
    ...
    it('GET /api/v1/services — lists service instances including the created one', () => {
      cy.request('/api/v1/services').then((response) => {
        ...
        const created = response.body.data.find((s: ServiceInstance) => s.id === createdServiceId);
    ...
    it('PATCH /api/v1/services/:id — merges config preserving existing fields', () => {
      cy.request({
        ...
        expect(response.body.config.baseUrl).to.eq(config.services.storage.baseUrl);
  ...
  describe('System default override', () => {
    ...
    it('system VC default shows isPrimary false when tenant has own primary', () => {
      cy.request('/api/v1/services?serviceType=VC').then((response) => {
        expect(response.status).to.eq(200);
        const systemVc = response.body.data.find((s: ServiceInstance) => s.id === SYSTEM_VC_SERVICE_ID);
        const tenantVc = response.body.data.find((s: ServiceInstance) => s.id === tenantPrimaryId);
        expect(systemVc).to.exist;
  ...
  describe('Filtering and pagination', () => {
    ...
    it('combines serviceType and adapterType filters', () => {
      cy.request('/api/v1/services?serviceType=VC&adapterType=VCKIT').then((response) => {
        expect(response.status).to.eq(200);
        response.body.data.forEach((s: ServiceInstance) => {
          expect(s.serviceType).to.eq('VC');
          expect(s.adapterType).to.eq('VCKIT');
