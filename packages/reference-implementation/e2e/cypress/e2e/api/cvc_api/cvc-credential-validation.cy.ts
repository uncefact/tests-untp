...
describe('CVC Credential Validation', { testIsolation: false }, () => {
   ...
   */
  function buildDccPayload(issuerDid: string, opts: { scopeId?: string; criteriaIds?: string[] } = {}) {
    const { scopeId, criteriaIds = [] } = opts;
    const uniqueSuffix = `${RUN_ID}-${Math.random().toString(36).slice(2, 8)}`;

    return {
      '@context': ['https://www.w3.org/ns/credentials/v2', 'https://test.uncefact.org/vocabulary/untp/dcc/0.6.1/'],
  ...
  before(() => {
    ...
    cy.apiLogin();
    cy.task('seedTestOrg', { userEmail: config.user.email })
      .then((result: any) => {
  ...
  describe('Missing criteria warnings', () => {
    it('returns CVC_MISSING_CRITERION when credential omits criteria required by its profile', () => {
      ...
      cy.request({
        ...
        const missingWarnings = response.body.warnings.filter((w: any) => w.code === 'CVC_MISSING_CRITERION');
  ...
  describe('Unknown criteria warnings', () => {
    it('returns CVC_UNKNOWN_CRITERION for criteria not in any imported catalogue', () => {
      const payload = buildDccPayload(defaultDidValue, {
        scopeId: PROFILE_CANONICAL_ID,
        criteriaIds: [CRITERION_1_CANONICAL_ID, CRITERION_2_CANONICAL_ID, 'https://example.com/unknown-criterion'],
      });
      ...
      cy.request({
        ...
        const unknownWarnings = response.body.warnings.filter((w: any) => w.code === 'CVC_UNKNOWN_CRITERION');
  ...
  describe('Scope warnings', () => {
    it('returns CVC_SCOPE_NOT_FOUND when scope does not match any imported profile', () => {
      ...
      cy.request({
        ...
        const scopeWarnings = response.body.warnings.filter((w: any) => w.code === 'CVC_SCOPE_NOT_FOUND');
    ...
    it('returns CVC_NO_CRITERIA when credential has scope but no assessment criteria', () => {
      ...
      cy.request({
        ...
        const noCriteriaWarnings = response.body.warnings.filter((w: any) => w.code === 'CVC_NO_CRITERIA');
