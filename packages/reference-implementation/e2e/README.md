...
- Harness controls: `E2E_RESIDUE_POLICY`, optional `E2E_RUN_ID`, the private-address capability inputs `FETCH_ALLOW_PRIVATE_URLS`, `VERIFY_ALLOW_PRIVATE_URLS`, `CYPRESS_VERIFY_ALLOW_PRIVATE_URLS`, the credential-status capability inputs, and the Node cleanup inputs `CREDENTIAL_STATUS_RECONCILE_GRACE_MS`, `E2E_STATUS_CLEANUP_TIMEOUT_MS`

The capability flags are `E2E_VCKIT_DID_WEB_RESOLVABLE`, the private-address setting, `CREDENTIAL_STATUS_MUTATION_ENABLED`, `CREDENTIAL_STATUS_DEFAULT_PURPOSES`, and `CREDENTIAL_STATUS_MULTIPLE_PURPOSES_ENABLED`. When the mutation flag is `false`, status reads still run but every status mutation journey skips, including the revoked-credential verification case. There is no database or object-store configuration: the suite never connects to either.

The E2E Compose stack does not default `CREDENTIAL_STATUS_MULTIPLE_PURPOSES_ENABLED` to `true`. The lifecycle spec therefore proves the shipped one-purpose default and the refusal of a two-purpose request.
...
| `E2E_VCKIT_PUBLIC_BASE_URL`                   | VCKit base URL as the test runner reaches it; specs that fetch a published status list rewrite the RI's internal origin to this one                                                                       | `http://localhost:3332`                                                     |
...
| `CREDENTIAL_STATUS_MUTATION_ENABLED`          | Application capability setting read literally by the harness. Set to `false` for a deployed instance where the status mutation kill switch is off; mutation specs skip while status reads remain covered. | Harness: `true` when unset; application: `false` unless explicitly enabled. |
| `CREDENTIAL_STATUS_DEFAULT_PURPOSES`          | Application default status purposes, declared to the harness so omitted-purpose capture assertions follow the deployment. Use `none` for no default entries.                                              | `revocation`                                                                |
| `CREDENTIAL_STATUS_MULTIPLE_PURPOSES_ENABLED` | Application capability for issuing more than one status purpose. The harness skips the two-purpose refusal when this is `true`.                                                                           | `false`                                                                     |
| `CREDENTIAL_STATUS_RECONCILE_GRACE_MS`        | Application recovery grace after a pending status deadline. The Node cleanup task waits for this grace when a tagged credential delete is interrupted.                                                    | `5000`                                                                      |
| `E2E_STATUS_CLEANUP_TIMEOUT_MS`               | Overall Node cleanup budget for pending status recovery. Cleanup never waits beyond this budget.                                                                                                          | `60000`                                                                     |
...
`CREDENTIAL_STATUS_MUTATION_ENABLED` is read literally from the root `.env`, `.env.e2e`, and the host environment, with the host environment winning. It must be `true` or `false`; the resolved Cypress capability must agree with the application setting. The E2E Compose app defaults it to `true`, while production configuration defaults it to `false`. For a deployed instance with mutation disabled, set `CREDENTIAL_STATUS_MUTATION_ENABLED=false` in the environment used by the app and Cypress so the capability guard skips mutation tests rather than treating the deployment as capable.

`CREDENTIAL_STATUS_DEFAULT_PURPOSES` and `CREDENTIAL_STATUS_MULTIPLE_PURPOSES_ENABLED` are read with the application's parsing rules and their declared capabilities must agree with the application settings. The default-purpose capability is `['revocation']` when unset and `[]` when set to `none`. The cleanup inputs are read by the Node task only. A pending delete is reconciled once after its deadline and grace when the cleanup budget permits; an unrecovered row is left in place and reported.
