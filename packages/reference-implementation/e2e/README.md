...
Before starting a local stack, remove `VERIFY_ALLOW_PRIVATE_URLS` from the root `.env`. If you set it deliberately, rename it to `FETCH_ALLOW_PRIVATE_URLS` without changing the value. Set exactly one boolean name in the shell used for both Compose and Cypress. The E2E Compose harness defaults private container addresses to allowed for local testing. To test a deployment with SSRF protection enabled, use `export FETCH_ALLOW_PRIVATE_URLS=false` instead.

```bash
# Use the same value for the application container and the Cypress harness.
export FETCH_ALLOW_PRIVATE_URLS=${FETCH_ALLOW_PRIVATE_URLS:-true}
...
| `FETCH_ALLOW_PRIVATE_URLS`          | Application setting used to initialise the harness's private-address capability. Set to `false` when testing a deployment that rejects private addresses.  | Harness: `true` if no effective input; application: `false` when unset. |
| `CYPRESS_VERIFY_ALLOW_PRIVATE_URLS` | Harness-only input for the retained `VERIFY_ALLOW_PRIVATE_URLS` capability key when no direct deprecated input is supplied. It does not configure the app. | Unset.                                                                  |

The application still accepts `VERIFY_ALLOW_PRIVATE_URLS` as a deprecated name during RI v0.5 when the deployment forwards it. Blank or whitespace-only application values count as unset. If neither application name is present, the harness defaults to `true`, unless `CYPRESS_VERIFY_ALLOW_PRIVATE_URLS` supplies another value. Both non-blank application names are a conflict and fail the application startup. See the [v0.5 migration guide](../../../documentation/docs/migration-guides/ri-v0.5#credential-fetch-settings-have-new-names) for the full mapping and warning period.

The four SSRF specs continue to call `Cypress.env('VERIFY_ALLOW_PRIVATE_URLS')`. This is the harness capability key and does not read the old application setting. A wrong harness value suppresses those SSRF cases silently because they skip, so keep the harness value aligned with the running app. For a strict deployment, pass `FETCH_ALLOW_PRIVATE_URLS=false` to the Cypress process.
