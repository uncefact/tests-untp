...
v0.6 adds issuer-owned credential status management to the Reference Implementation. It also makes the status-operation boundary visible in the library and gives conformity score checks a clearer advisory contract. Read the [v0.6 Reference Implementation migration guide](https://uncefact.github.io/tests-untp/docs/migration-guides/ri-v0.6) before upgrading.

- **Batch credential issuance:** Issuers can submit many credential requests in one call with `POST /api/v1/credentials/batches` under an `Idempotency-Key`, and follow state, counts and per-item outcomes with `GET /api/v1/credentials/batches/{id}`. The worker issues items through the same path as single issuance. A refused item is recorded and the batch continues, an item that faults before the issuance call retries with backoff up to four attempts, and an item whose outcome is unknown is held for the operator, who inspects and resolves it with `batch:inspect-item` and `batch:resolve-item`. Settled batches expire after `BATCH_RETENTION_DAYS`. See the [Credential API](https://uncefact.github.io/tests-untp/docs/reference-implementation/api/credentials#batch-issuance) and the [batch issuance runbook](https://uncefact.github.io/tests-untp/docs/reference-implementation/operations/batch-issuance).
- **Credential status management:** Issuers can read stored or fresh status observations with `GET /api/v1/credentials/{id}/status`, change one purpose with `PUT /api/v1/credentials/{id}/status/{purpose}`, and record a provider observation or recover an uncertain change with `POST /api/v1/credentials/{id}/status/{purpose}/reconcile`. Clients must send the entry version in `If-Version` and treat an unconfirmed result as work to reconcile, not as proof that the change failed. See the [Credential API](https://uncefact.github.io/tests-untp/docs/reference-implementation/api/credentials#issuer-status).
