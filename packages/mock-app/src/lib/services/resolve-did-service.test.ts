...
jest.mock("@/lib/prisma/repositories", () => ({
  getInstanceByResolution: (...args: unknown[]) =>
    mockGetInstanceByResolution(...args),
}));
...
describe("resolveDidService", () => {
  ...
  it("resolves DID service from system default", async () => {
    ...
    expect(mockGetInstanceByResolution).toHaveBeenCalledWith(
      "org-1",
      "DID",
      undefined,
    );
  ...
  it("throws ServiceInstanceNotFoundError for explicit ID not found", async () => {
    ...
    await expect(
      resolveDidService("org-1", "missing-id"),
    ).rejects.toThrow(ServiceInstanceNotFoundError);
  });

  it("throws ServiceResolutionError when no instance found", async () => {
    mockGetInstanceByResolution.mockResolvedValue(null);

    await expect(resolveDidService("org-1")).rejects.toThrow(
      ServiceResolutionError,
    );
  });
  ...
  it("throws ConfigValidationError on schema validation failure", async () => {
    ...
    mockConfigSchema.safeParse.mockReturnValue({
      ...
      error: {
        issues: [
          ...
          { message: "authToken must be a string" },
        ],
      },
    });
    ...
    await expect(resolveDidService("org-1")).rejects.toThrow(
      /endpoint is required, authToken must be a string/,
    );
  });
