...
import {
  ...
  ComponentType,
} from '../components/DynamicComponentRenderer/DynamicComponentRenderer';
...
describe('render DynamicComponentRenderer component', () => {
  ...
  it("Should update the dynamically loaded component when the 'name' prop changes", () => {
    ...
    const { rerender } = render(
      <DynamicComponentRenderer name='ImportButton' type={ComponentType.EntryData} props={{}} />,
    );
    ...
    rerender(
      <DynamicComponentRenderer name='RenderCheckList' type={ComponentType.EntryData} props={{ test: 'test' }} />,
    );
