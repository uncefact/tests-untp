import { getQrLink } from './implementations';
...
export async function getAndParseQrLink(filePath: string) {
  const qrLink = await getQrLink(require(filePath));
  return parseQRLink(qrLink);
}
