...
Service instances exist at two levels — **system services** (created during [startup](../operations/startup#step-2-database-seed), owned by the [system tenant](../system-architecture#system-tenant), read-only defaults visible to all tenants) and **tenant services** (created through this API, scoped exclusively to the creating tenant). See [System Services vs Tenant Services](../services/service-architecture#system-services-vs-tenant-services) for details.
