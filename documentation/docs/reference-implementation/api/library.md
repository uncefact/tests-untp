...
The `checks` object always lists seven checks (`retrieval`, `decryption`, `digest`, `proof`, `status`, `temporal`, `schemaConformance`), each `pass`, `fail` or `not_run`. `digest` answers a different question depending on how the generation acquired its bytes: on a source acquisition (a registration, or a recovery that fetched the supplier source) it reports whether the acquired bytes hash to the declared digest, and on a stored-copy acquisition (a key-bearing recovery of the record's own unopened copy) it reports whether that copy matched the integrity digest recorded for it. Whether the body has credential shape belongs to `proof` and schema conformance. Both are blocking, and a record can carry one meaning at one generation and the other at the next. The summary of a `complete` generation is derived from those published checks. Any failed blocking check makes it `not_conformant`. Otherwise it is `verified` only when `proof` passed; a generation whose proof was not established (the verifier never reached the signature, or the run never reached the verifier) is `not_conformant`. The blocking checks are `retrieval`, `decryption`, `digest`, `proof` and `status`. `temporal` is recorded as evidence and does not block, so a genuine credential that has expired is still `verified` with `temporal: fail`. Its currency is reported separately in `currencyStatus`. The temporal check is judged from the credential's own `validFrom` and `validUntil` as well as by the verification provider, so it does not depend on the provider enforcing the window. A native record's first generation is an issuance assertion: `proof: pass` with `status: not_run`, which says the service issued and signed it and says nothing about revocation since; re-verify the record for a current status check. `schemaConformance` is advisory and never changes the summary.

`schemaConformance` runs on worker-settled verification generations after the record's details have been extracted. It validates the decoded credential against the system core schema for its stored core type and data model version, then expands its JSON-LD using the credential's declared contexts. It does not validate extension schemas. Closed definitions nested inside the core schema can still make the check fail. The schema and context loads use the normal network guard and the bundled UNTP artefact fallback; see [Bundled UNTP Artefacts](../operations/bundled-untp-artefacts). A native record's generation 1 is its issuance assertion and remains `not_run`; native re-verification generations use the worker check. Issuance validates against the tenant-visible data model, whereas this advisory check uses the system core schema, so a native re-verification may report an advisory failure for a credential that issuance accepted.

The result is `pass` when both validation stages complete successfully. It is `fail` when the core schema or JSON-LD document is invalid, and the result is published as a `SCHEMA_CONFORMANCE_ADVISORY` entry in the record's `warnings` array. The warning is shown only for a settled newest generation, never while that generation is pending. The message for a schema violation names the pointer and validator's text; a JSON-LD document fault names the fault kind only, never a value. An unrecognised expansion failure uses a generic document-fault message. A malformed term in a fetched remote context and an unrecognised processor error are classifier residuals that are both read as a document failure. A recorded message is truncated at 1024 characters. It is `not_run` when details were not extracted, the stored copy could not be decoded after extraction, the schema or a context could not be obtained, or was obtained and could not be used, after fallback, or the stage allowance was exhausted. These outcomes do not alter the other checks. The check shares the attempt's time budget with the verifier and runs first, so prolonged schema or context work can leave the verifier no allowance, in which case the attempt fails as `VERIFICATION_UNAVAILABLE` and retries. The conformance module's own log lines carry the artefact origin, the stage, the error name or code and, for an invalid context, the offending field names; the shared bundled-fallback logger, which the worker now also triggers, logs the public core artefact URL and the fetch error as an operator diagnostic (see [Bundled UNTP Artefacts](../operations/bundled-untp-artefacts)). The JSON-LD obtain-failure log carries a context URL's origin only when the diagnostic names a URL, and otherwise the diagnostic's `errorCode` and, when present, `fields`. The schema obtain-failure log always carries the schema origin and the error's name, because this service built that URL itself.
...
sequenceDiagram
    ...
    Client->>RI: GET /api/v1/library?type=...&sort=...&page=...
    RI->>RI: Validate filters, sort and paging
    RI->>DB: Query the tenant's records with filter, sort and offset pushed down
    DB-->>RI: Matching rows and total count
    RI-->>Client: 200 OK ({ data, pagination }), Cache-Control: no-store
```
...
The `lifecycle` filter is independent of the verification `status` filter. Both are applied before counts and pagination, with status entries correlated to the record and its tenant. `status=failed&lifecycle=revoked` can return a revoked record whose newest verification failed operationally.

`status=verified&lifecycle=none` means verification passed and no confirmed lifecycle bit is set. It can still return a record with `STATUS_CHANGE_UNCONFIRMED`, an unobserved lifecycle entry alongside a clear one, or no lifecycle entries at all. Native generation 1 is an issuance assertion whose status check was not run. Currency, temporal results and observation freshness remain the consumer's policy; the query does not promise that a credential is usable.
...
sequenceDiagram
    ...
    Client->>RI: POST /api/v1/library/batch-get { ids }
    RI->>RI: Validate the id list against API_MAX_BATCH_LIMIT
    RI->>DB: Load the tenant's records for those ids
    DB-->>RI: Rows found
    RI-->>Client: 200 OK (found rows, ids not found listed separately)
```
...
Malformed JSON, a missing or invalid `ids` array, and an over-limit request return `400`. An oversized body returns `413 REQUEST_BODY_TOO_LARGE` under the [request body size limit](../operations/api-pagination#request-body-size). Authentication and tenant-assignment failures keep the shared `401` and `403` responses. A database, transaction or selection-boundary failure returns a sanitised `500` with a correlation id; a row-local read or projection failure is a `RECORD_UNREADABLE` entry in `failures`.
...
sequenceDiagram
    participant Client
    participant RI as Reference Implementation (web)
    participant Storage as Storage Service
    participant Queue as Job Queue (database)
    participant Worker as Reference Implementation (worker)
    participant VC as Verifiable Credential Service

    Client->>RI: POST /api/v1/library { url }
    RI->>RI: Guarded fetch of the credential, decrypt if a key was supplied
    RI->>RI: Extract details, compute digest
    RI->>Storage: Store the durable copy (encrypted)
    Storage-->>RI: storageUri
    RI->>Queue: Enqueue verification job (generation 1, pending)
    RI-->>Client: 201 record with verification.state = pending
    Worker->>Queue: Claim job
    Worker->>Storage: Read the copy back, check digest
    Worker->>VC: Verify proof and status
    VC-->>Worker: Result
    Worker->>Worker: Temporal and schema conformance checks
    Worker->>Queue: Settle generation (complete or failed)
    Client->>RI: GET /api/v1/library/{id}
    RI-->>Client: 200 record with the settled envelope
```
...
sequenceDiagram
    ...
    Client->>RI: GET /api/v1/library/{id}
    RI->>DB: Load the tenant's record with its newest generation
    DB-->>RI: Record, custody and annotations
    RI->>RI: Reveal the held key when it can be opened, else DECRYPTION_KEY_UNAVAILABLE warning
    RI-->>Client: 200 OK (record with verification envelope), Cache-Control: no-store
```
...
sequenceDiagram
    ...
    Client->>RI: PATCH /api/v1/library/{id} (If-Version, annotations)
    RI->>DB: Load the tenant's record
    DB-->>RI: Record and annotationVersion
    RI->>RI: Refuse a native record (403), compare If-Version (409 on mismatch)
    RI->>DB: Write annotations, increment annotationVersion
...
sequenceDiagram
    ...
    Client->>RI: POST /api/v1/library/{id}/verify (optional key)
    RI->>RI: Check eligibility and the newest generation's state
    RI->>Queue: Reserve generation N+1 as pending and enqueue the job
    RI-->>Client: 202 Accepted (record with the pending generation)
    Worker->>Queue: Claim job, run the checks, settle the generation
    Client->>RI: GET /api/v1/library/{id}
    RI-->>Client: 200 OK (settled envelope)
```
...
The accepted bodies are either zero bytes, which is the bodyless form, or a JSON object containing `sourceEncryption.decryptionKey` as a 64-character hexadecimal key. The validator strips unknown fields after requiring that documented key, so `{}` is refused with `400 VALIDATION_FAILED`; it is not the bodyless form. Malformed JSON, whitespace, an invalid key, or a padded key is also refused before the record is read. A body that cannot be read and an over-sized body keep the inherited uncoded `400` and `413 REQUEST_BODY_TOO_LARGE` responses described in the Swagger operation. The supplied key is used only for this request and is never stored, logged, queued or returned.
...
9. Otherwise the record is recovered in the request. It always reserves generation N+1 as `PENDING` first, under the parent's lock, then acquires the bytes outside any transaction. A concurrent **bodyless** request that reads the reservation joins it and starts no second acquisition; a concurrent key-bearing one is refused by step 5 instead. A bodyless request that read the record _before_ the reservation existed does not reach that join: it evaluates step 6 against the copy it read, so on a record holding unopened ciphertext it is refused `400 DECRYPTION_REQUIRED` and its next step is to send the key, not to poll the generation that won. The reservation returns the custody it observed under that lock, and **that** snapshot chooses the mode, not the read the request entered with, so a record that acquired a durable copy between the two takes mode B against the copy that now exists. A generation that completed while the record stayed eligible reserves the **next** generation rather than answering with the superseding one, so a supplied key is always consumed or refused by name. Recovery readies the job queue immediately after a successful reservation and before the acquisition. A queue that will not start _before_ this reservation exists (the native and protected-copy branches above, which enqueue before locking anything) still answers a sanitised `500` with no generation created. A queue that will not start _after_ the reservation exists instead settles it `FAILED VERIFICATION_UNAVAILABLE` (retryable) with nothing acquired or stored, and answers `202` with that settled generation, because the reservation itself already exists; a later re-verify reserves a fresh one. That `202` is only truthful once the settle write itself actually committed: if the settle write itself also fails (a transient database fault, separate from the queue failure that triggered it), the reservation stays `PENDING` and the request answers the queue's own sanitised `500` instead of a false promise of a settled generation, with the reconciliation sweep as the eventual backstop for a reservation neither path could settle.
   ...
   **Mode A**, a record with no durable copy, fetches `sourceUrl` in the request, with the supplied key when one was sent. **Mode B**, a record holding an unopened durable copy, reads the reserved `storageUri` through the storage transport, checks the raw bytes against `storageDigestMultibase`, and opens them with the supplied key. Mode B never fetches `sourceUrl` and never performs a freshness check. What the acquisition returns, not the record's prior state, decides everything from here:
   ...
   The in-request read of the record's own durable copy is bounded by `WORKER_JOB_TIMEOUT_SECONDS`, which defaults to 300 seconds. The caller's HTTP timeout should allow for that bound. This replaced a fixed 10-second bound.
...
sequenceDiagram
    ...
    Client->>RI: DELETE /api/v1/library/{id}
    RI->>DB: Load the tenant's record
    RI->>RI: Refuse a native record (403 NATIVE_CREDENTIAL_NOT_DELETABLE)
    RI->>DB: Delete the record, its generations and its registration claim
    RI->>Storage: Delete the durable copy (best effort, after the commit)
