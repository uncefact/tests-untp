...
The result is `pass` when both validation stages complete successfully. It is `fail` when the core schema or JSON-LD document is invalid, and the result is published as a `SCHEMA_CONFORMANCE_ADVISORY` entry in the record's `warnings` array. The warning is shown only for a settled newest generation, never while that generation is pending. The message identifies the first JSON Schema violation, or the detail and code supplied by a recognised JSON-LD diagnostic. An unrecognised expansion failure uses the classifier's generic detail. A malformed term in a fetched remote context and an unrecognised processor error are classifier residuals that are both read as a document failure. A recorded message is truncated at 1024 characters. It is `not_run` when details were not extracted, the stored copy could not be decoded after extraction, the schema or a context could not be obtained, or was obtained and could not be used, after fallback, or the stage allowance was exhausted. These outcomes do not alter the other checks. The JSON-LD obtain-failure log carries a context URL's origin only when the diagnostic names a URL, and otherwise the diagnostic's source and code. The schema obtain-failure log always carries the schema origin and the error's name, because this service built that URL itself. No URL path, payload value or full diagnostic message is placed in a record or log.
