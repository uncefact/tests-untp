...
The result is `pass` when both validation stages complete successfully. It is `fail` when the core schema or JSON-LD document is invalid. The message identifies the first JSON Schema violation, or the detail and code supplied by a recognised JSON-LD diagnostic. An unrecognised expansion failure uses the classifier's generic detail. It is `not_run` when details were not extracted, the stored copy could not be decoded after extraction, the schema or a context could not be obtained after fallback, or the stage allowance was exhausted. These outcomes do not alter the other checks. An obtain-failure log carries a URL origin only when the diagnostic has a URL; otherwise it carries the diagnostic source and code. No URL path, payload value or full diagnostic message is placed in a record or log.
