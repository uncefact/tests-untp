...
| `statusPurposes`                           | string[] | No       | Status purposes requested from the VC service. Only one purpose is allowed by default because UNTP v0.7.0 schemas accept one `credentialStatus` object. Set `CREDENTIAL_STATUS_MULTIPLE_PURPOSES_ENABLED=true` to allow more than one. The Reference Implementation supports `revocation` and `suspension` (ADR-058); the underlying provider may support more. Values must be unique and non-empty when supplied. An empty array is refused because a per-request no-status choice is not offered. When omitted, the deployment's `CREDENTIAL_STATUS_DEFAULT_PURPOSES` applies; `none` alone means no status entries, and if the variable is unset the built-in default is `['revocation']`. |
| `storageOptions.serviceInstanceId`         | string   | No       | Explicit storage service instance. If provided, it must be accessible to the tenant (its own, or a system default); otherwise the request is rejected with `404 SERVICE_INSTANCE_NOT_FOUND`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
Every field is shape-checked at the boundary: a missing or mistyped field is rejected with a 400 that names it, and unknown fields are ignored. The verification URLs must be well-formed HTTP(S) URLs without embedded credentials, `hreflang` entries must be well-formed [BCP 47](https://www.rfc-editor.org/rfc/rfc5646.html) language tags, and `linkType` must not be blank. Passing `null` for `storageOptions` or `publishingOptions` is rejected; omit them instead. The batch-only `reference` field is discarded here.

The VC service signs the credential with one status entry for each requested purpose. The Reference Implementation supports `revocation` and `suspension` (ADR-058), while the underlying provider may support more. Omitting `statusPurposes` uses the deployment's `CREDENTIAL_STATUS_DEFAULT_PURPOSES` setting, or the built-in `revocation` default when the variable is unset or blank. By default, a request can name only one purpose because UNTP v0.7.0 schemas accept one `credentialStatus` object. Set `CREDENTIAL_STATUS_MULTIPLE_PURPOSES_ENABLED=true` before sending both supported purposes in `"statusPurposes": ["revocation", "suspension"]`. A caller-supplied `credentialPayload.credentialStatus` is refused with `CREDENTIAL_STATUS_NOT_ACCEPTED`; the Reference Implementation mints and manages status entries.
...
- `STATUS_LIST_BUSY` when the wait for the status-list mutex expires, bounded by `CREDENTIAL_STATUS_LOCK_ACQUIRE_MS`, or when coordination capacity is unavailable. Retry shortly.
...
Use the native record id. Do not use the signed credential's identifier. The stored response contains `capture`, `statusCaptureError`, `attribution` and `entries`. `attribution` contains `instanceId`, `source` and `at`, or it is `null`.

Each entry contains `entryId`, `statusPurpose`, its last confirmed `value`, `observedAt`, `valueChangedAt`, `version` and `pending`. `value` is a boolean or `null`. `pending` contains `value`, `since` and `deadline`, or it is `null`.

An unobserved entry has `value: null`. This does not mean that the bit is clear. The [library record](./library#issuer-lifecycle) describes the capture states.

`fresh=true` adds separate `observed` and `failures` arrays. Observations contain `entryId`, `statusPurpose`, `value` and `observedAt`.

Failures name the entry, purpose, error `code` and `message`. A provider error never becomes a bit value. The read uses the recorded issuing instance. It uses the pinned instance while a change is pending.

The read writes nothing. It does not advance a version or resolve a pending change. Both forms answer with `Cache-Control: no-store`.

Missing or foreign records answer `404 NOT_FOUND`. External records answer `403 EXTERNAL_CREDENTIAL_STATUS_NOT_MANAGEABLE`. An invalid `fresh` value answers `400 VALIDATION_FAILED`.

Authentication requires a valid tenant identity, as on the other credential routes. A malformed native record answers `500 RECORD_UNREADABLE`. Database failures use the shared sanitised `500` response.
...
Select the purpose in the path. The credential must already carry that entry. Only `revocation` and `suspension` can be changed.

Revocation is irreversible. `value: false` is refused even when the previously observed bit was clear. Suspension can be set and cleared. Historical `refresh` entries are also irreversible. They are refused as unsupported before any transition is considered.

Entries larger than one bit cannot be changed. Mutation must be [enabled by the operator](../operations/credential-status-recovery#enable-status-changes).

First read the status entry's `version` and send it in `If-Version`. This is the entry version. It is not an annotation version or verification generation.

A stale request is refused. It is never replayed. Unknown body keys are ignored. `value` must be a JSON boolean. The path purpose is decoded once. It must contain 1 to 255 characters and cannot contain control characters.
...
Before contacting the provider, the service records durable pending intent, its issuing instance, configuration digest and deadline. One deadline bounds the preliminary read, lock acquisition, set and read-back.

If the preliminary read already equals the requested value, the service commits that observation without setting the bit again. Otherwise the change succeeds only when the read-back matches the request. The commit must also still hold the same reservation token and entry version.
...
Other database failures before dispatch use the shared sanitised `500` response. Deletion and effective service-configuration changes are blocked while any pending intent exists. [Recovery guidance](../operations/credential-status-recovery) covers an uncertain write and an unreachable pinned provider.
...
Reconciliation reads the provider and records what it observes. It never sets a bit.

With pending intent, it waits until the recorded deadline plus `CREDENTIAL_STATUS_RECONCILE_GRACE_MS`. It reads the pinned issuing instance. It commits only when the inspected token and version still match.

Before that time it answers `409 STATUS_OPERATION_IN_PROGRESS`. A failed read normally answers `503 VC_SERVICE_UNAVAILABLE`. Malformed stored status input answers `500 RECORD_UNREADABLE`. An unrepresentable management entry answers `422 STATUS_ENTRY_UNSUPPORTED`. An invalid provider response answers `502 VC_STATUS_RESPONSE_INVALID`.

These failures leave pending intent and confirmed facts unchanged. The deterministic 422 and 502 cases state that retrying will not help.

If the current provider configuration differs from the original pin, the response is `503 STATUS_PROVIDER_CHANGED`. Establish that the replacement configuration identifies the intended provider. Then send `{ "acceptProviderChange": true }` explicitly. An operator's configuration repair preserves the original digest. It does not supply this acknowledgement on your behalf.

With no pending intent, reconciliation establishes a first or later observation using the recorded attribution. It skips the pending deadline and digest comparison. It rechecks attribution and configuration before committing.

A concurrent reservation blocks persistence even when the version did not change. Both branches advance the entry version. Both return the same observation shape as a successful change. Only a successful pending reconciliation clears the inspected intent.

Reconciliation separates a deterministic fault from an unavailable provider. Only the unavailable provider is worth retrying.

An entry this service cannot represent answers `422 STATUS_ENTRY_UNSUPPORTED`. An unreadable provider observation answers `502 VC_STATUS_RESPONSE_INVALID`. Both leave pending intent unchanged. Both answer the same way on a repeat. Resolve the entry or the provider before reconciling again.
