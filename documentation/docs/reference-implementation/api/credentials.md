...
For an issuer that needs to issue several credentials, `POST /api/v1/credentials/batches` accepts an ordered `items` array. Every item uses the exact request body shape accepted by `POST /api/v1/credentials`, including refusal of caller-supplied `credentialPayload.credentialStatus`. The batch boundary validates that shape, the configured item count, each item's serialised JSON size, the whole request body size and that the array is non-empty. DID ownership, service resolution, JSON-LD and schema conformance run for each item in the worker, so one item can fail without stopping the rest.
