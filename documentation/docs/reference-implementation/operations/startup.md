...
5. The worker's settings are read and checked: the reconciliation cadence (`LIBRARY_RECONCILE_PENDING_RUNS_CRON`), the per-tick cap (`LIBRARY_RECONCILE_PENDING_RUNS_BATCH_SIZE`) and `WORKER_JOB_TIMEOUT_SECONDS` (default 300 seconds, range 30 seconds to 24 hours). The last setting is the expiry and working budget for every job the queue sends or schedules. The web process reads the same setting when it constructs its queue, so both services must carry the same value. The web process reads the same setting twice: when it constructs its queue, and inside the key-bearing recovery on POST /api/v1/library/{id}/verify, which bounds its read of the record's own durable copy by the same number of seconds. Both services must therefore carry the same value, and raising it for a slow worker also raises what a caller can wait on that route. A value that cannot be used fails the boot with `worker.configuration-invalid`, naming the variable, before the queue exists.
