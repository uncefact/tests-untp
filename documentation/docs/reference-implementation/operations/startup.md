...
For the web role, it validates `RI_APP_URL`, `RI_HTTP_USER_AGENT`, `CACHE_MAX_ENTRIES`, `BUNDLED_ARTEFACTS_FALLBACK`, `IDEMPOTENCY_STALE_CLAIM_MINUTES`, `MAX_REQUEST_BODY_BYTES`, `FETCH_ALLOW_PRIVATE_URLS`, `VERIFY_ALLOW_PRIVATE_URLS`, `FETCH_MAX_RESPONSE_SIZE`, `VERIFY_MAX_CREDENTIAL_SIZE`, `FETCH_TIMEOUT_MS`, `VERIFY_FETCH_TIMEOUT_MS`, `CVC_REFRESH_INTERVAL_HOURS`, `DEFAULT_STATUS_PURPOSES`, `STATUS_LOCK_ACQUIRE_MS`, `STATUS_OPERATION_BUDGET_MS`, `STATUS_RECONCILE_GRACE_MS`, `STATUS_MUTATION_ENABLED`, and `WORKER_JOB_TIMEOUT_SECONDS`.
...
When `DEFAULT_STATUS_PURPOSES=none`, the preflight logs this warning: `DEFAULT_STATUS_PURPOSES=none: credentials issued without an explicit statusPurposes carry no status entry and can never be revoked or suspended.`
...
| ---------------------------- | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `STATUS_OPERATION_BUDGET_MS` | `30000` | Positive decimal integer, at most `2147483647`. Bounds preliminary read, lock acquisition, set and read-back with one deadline. Reconcile and fresh status reads also use this budget.                                                             |
| `STATUS_RECONCILE_GRACE_MS`  | `5000`  | Positive decimal integer, at most `2147483647`. Added to a pending deadline before reconciliation is admitted. It does not automatically clear intent.                                                                                             |
| `STATUS_MUTATION_ENABLED`    | `false` | Exact `true` or `false`; unset or empty disables mutation. The lock identity changes at process start for issuance and status sets. The compatibility lock serialises mixed old and new processes, and a drain is still recommended before deploy. |

Invalid non-empty settings fail web boot before requests are accepted. The two durations use their defaults when unset or blank. Both Compose definitions forward these values to the web service only. The worker does not perform issuer status mutations. `STATUS_LOCK_ACQUIRE_MS` continues to bound lock acquisition within the operation's remaining allowance; status changes distinguish contention (`STATUS_LIST_BUSY`) from unavailable coordination capacity (`STATUS_COORDINATION_UNAVAILABLE`). See [Credential status recovery](./credential-status-recovery) for enablement, the locking limit and recovery after an uncertain outcome.
