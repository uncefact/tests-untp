...
5. The worker's settings are read and checked: the reconciliation cadence (`LIBRARY_RECONCILE_PENDING_RUNS_CRON`), the per-tick cap (`LIBRARY_RECONCILE_PENDING_RUNS_BATCH_SIZE`) and `WORKER_JOB_TIMEOUT_SECONDS` (default 300 seconds, maximum 86400). The last setting is the expiry and working budget for every job the queue sends or schedules. The web process reads the same setting when it constructs its queue, so both services must carry the same value. A value that cannot be used fails the boot with `worker.configuration-invalid`, naming the variable, before the queue exists.
