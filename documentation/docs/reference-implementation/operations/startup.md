...
### Credential Fetch Timeout Validation

`VERIFY_FETCH_TIMEOUT_MS` bounds one fetch of a caller-supplied credential URL, end to end, on the verify route, on registering an external library credential, and on the supplier-source check a re-verification request makes (default 10000, maximum 120000). All three run in the web process. When it is set, startup validates it is a positive integer within that ceiling and fails with a message naming the variable otherwise.
...
The settlement is state-guarded. A job that finishes after the sweep cannot overwrite the failure, and a later bodyless re-verification creates the next generation. The web process never schedules or consumes this queue. Each cron tick becomes one job that one worker fetches, so replicas do not each run the same reconciliation tick.
