...
Both roles validate `MAX_BATCH_ITEMS`, `BATCH_RETENTION_DAYS` and `BATCH_EXPIRY_SWEEP_MINUTES` before starting. The web role also validates `MAX_BATCH_REQUEST_BODY_BYTES` and refuses it when it is smaller than `MAX_REQUEST_BODY_BYTES`. `BATCH_JOB_CONCURRENCY` is read by the worker only, when it loads its configuration, and a value that is not a positive integer fails the worker before the queue starts.
...
`CREDENTIAL_STATUS_LOCK_ACQUIRE_MS` sets the credential-issuance wait for the status-list transaction mutex, in milliseconds. When set, it must be a positive integer. The mutex's built-in default is 2,000 milliseconds, from `DEFAULT_ACQUIRE_MS` in `packages/reference-implementation/src/lib/services/status-list-mutex.ts`. If acquisition expires, issuance answers `503 STATUS_LIST_BUSY`. An invalid or non-positive value logs one warning naming the supplied value and the 2,000-millisecond default, then the mutex uses its own fallback.
