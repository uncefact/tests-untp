...
For the web role, it validates `RI_APP_URL`, `RI_HTTP_USER_AGENT`, `CACHE_MAX_ENTRIES`, `BUNDLED_ARTEFACTS_FALLBACK`, `IDEMPOTENCY_STALE_CLAIM_MINUTES`, `MAX_REQUEST_BODY_BYTES`, `MAX_BATCH_REQUEST_BODY_BYTES`, `MAX_BATCH_ITEMS`, `BATCH_JOB_RETRY_LIMIT`, `BATCH_JOB_RETRY_BACKOFF_SECONDS`, `BATCH_JOB_RETRY_BACKOFF_MAX_SECONDS`, `FETCH_ALLOW_PRIVATE_URLS`, `VERIFY_ALLOW_PRIVATE_URLS`, `FETCH_MAX_RESPONSE_SIZE`, `VERIFY_MAX_CREDENTIAL_SIZE`, `FETCH_TIMEOUT_MS`, `VERIFY_FETCH_TIMEOUT_MS`, `CVC_REFRESH_INTERVAL_HOURS`, `CREDENTIAL_STATUS_DEFAULT_PURPOSES`, `CREDENTIAL_STATUS_LOCK_ACQUIRE_MS`, `CREDENTIAL_STATUS_OPERATION_BUDGET_MS`, `CREDENTIAL_STATUS_RECONCILE_GRACE_MS`, `CREDENTIAL_STATUS_MUTATION_ENABLED`, `CREDENTIAL_STATUS_MULTIPLE_PURPOSES_ENABLED`, and `WORKER_JOB_TIMEOUT_SECONDS`. It does not validate the worker-only expiry sweep cadence or retention period.
For the worker role, it validates `RI_HTTP_USER_AGENT`, `CACHE_MAX_ENTRIES`, `BUNDLED_ARTEFACTS_FALLBACK`, `MAX_BATCH_ITEMS`, `BATCH_RETENTION_DAYS`, `BATCH_EXPIRY_SWEEP_MINUTES`, `BATCH_JOB_RETRY_LIMIT`, `BATCH_JOB_RETRY_BACKOFF_SECONDS`, `BATCH_JOB_RETRY_BACKOFF_MAX_SECONDS`, `BATCH_JOB_CONCURRENCY`, `WORKER_JOB_TIMEOUT_SECONDS`, `LIBRARY_RECONCILE_PENDING_RUNS_CRON`, `LIBRARY_RECONCILE_PENDING_RUNS_BATCH_SIZE`, and the required `DATA_ENCRYPTION_KEY`; it skips the web-only settings. At worker boot, after the shared preflight, `resolveWorkerConfiguration` validates `BATCH_SETTLEMENT_ALLOWANCE_MS` and `BATCH_MINIMUM_ITEM_COST_MS`.
Both roles validate `DATA_ENCRYPTION_KEY` and `SERVICE_ENCRYPTION_KEY` naming and consistency, the encryption key format and placeholder policy using `DEPLOYMENT_ENVIRONMENT`, `LOG_REDACT_PATHS`, and these OTLP exporter settings when set: `OTEL_EXPORTER_OTLP_ENDPOINT`, `OTEL_EXPORTER_OTLP_HEADERS`, `OTEL_EXPORTER_OTLP_TRACES_HEADERS`, `OTEL_EXPORTER_OTLP_LOGS_ENDPOINT`, `OTEL_EXPORTER_OTLP_LOGS_HEADERS`, `OTEL_EXPORTER_OTLP_LOGS_PROTOCOL`, `OTEL_EXPORTER_OTLP_METRICS_ENDPOINT`, `OTEL_EXPORTER_OTLP_METRICS_HEADERS`, `OTEL_EXPORTER_OTLP_METRICS_PROTOCOL`, and `OTEL_EXPORTER_OTLP_PROTOCOL`.
The entrypoint derives the role from its first argument marker: `--process-role=worker` means worker, no marker means web, and any other marker value is refused.
Recognised maintenance commands invoked with `docker compose run`, including `audit:encryption`, `rotate:encryption-key`, `backfill:decryption-keys`, `backfill:credential-details`, `backfill:credential-status-entries`, `backfill:credential-status-attribution`, the digest backfill, `batch:resolve-item` and `batch:inspect-item`, do not run this preflight; their own configuration checks apply, while `SKIP_MIGRATIONS=true` and `SKIP_SEED=true` still skip migrations and the seed. Batch inspection and resolution run in the web container with `docker compose exec`.
...
When `CREDENTIAL_STATUS_DEFAULT_PURPOSES=none`, the preflight logs this warning: `CREDENTIAL_STATUS_DEFAULT_PURPOSES=none: credentials issued without an explicit statusPurposes carry no status entry and can never be revoked or suspended.`
...
Each version of the Reference Implementation may include changes to the database schema, including new tables, new columns, or modified constraints. Migrations apply these changes so that the database matches the version of the application being started.
...
| Default DID        | A system Decentralised Identifier (DID) created via the verifiable credential service. `SYSTEM_DID_KEY_ID` selects the verification method to record when set. `SYSTEM_DID_NAME` and `SYSTEM_DID_DESCRIPTION` set its display values                                                  | `SYSTEM_DID`, optional `SYSTEM_DID_KEY_ID`, `SYSTEM_DID_NAME`, `SYSTEM_DID_DESCRIPTION`, and `SYSTEM_VC_*` variables |
...
When `SYSTEM_DID_KEY_ID` is set, it must identify a verification method in the `SYSTEM_DID` document and that key ID is recorded for the system default DID. When it is omitted, the seed uses the key ID returned by the VC service when it creates the DID, or the first verification method when the DID already exists. An existing DID document with no verification methods causes the seed to fail and asks you to set `SYSTEM_DID_KEY_ID` explicitly.
...
The seed script is located at `packages/reference-implementation/prisma/seed-cli.ts` (which runs the logic in `prisma/seed.ts`) in the [repository](https://github.com/uncefact/tests-untp). Organisations that need additional seed data, including custom registrars, identifier schemes, data models, render templates, or conformity schemes, supply it via a Docker volume mount, without modifying the source. See [Custom Seed](./custom-seed.md).
...
The shared [request body size limit](./api-pagination#request-body-size) documents the configured cap and the `413 REQUEST_BODY_TOO_LARGE` response.

### Batch Issuance Settings

The batch endpoints and worker use these settings. The raw batch body bound is web-only; the other batch settings are boot-validated as shown below:

| Variable                              | Description                                                                                                                                                                                                                                                                                                      | Default    |
| ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| `MAX_BATCH_REQUEST_BODY_BYTES`        | Maximum raw request body size for one batch submission. Must be a positive integer and at least `MAX_REQUEST_BODY_BYTES`. The web process only.                                                                                                                                                                  | `52428800` |
| `MAX_BATCH_ITEMS`                     | Maximum number of credential request items accepted by one batch submission. Must be a positive integer.                                                                                                                                                                                                         | `500`      |
| `BATCH_RETENTION_DAYS`                | Number of days a settled batch remains available before its items are deleted and an `EXPIRED` tombstone remains. Must be a positive integer.                                                                                                                                                                    | `30`       |
| `BATCH_EXPIRY_SWEEP_MINUTES`          | Minutes between worker sweeps that delete item data from due settled batches. Must be less than 60 or a multiple of 60.                                                                                                                                                                                          | `60`       |
| `BATCH_JOB_RETRY_LIMIT`               | Number of queue retries after the first fault and the per-item pre-dispatch fault limit. Must be a positive integer no greater than `2147483647`.                                                                                                                                                                | `4`        |
| `BATCH_JOB_RETRY_BACKOFF_SECONDS`     | Starting delay for queue retries and the per-item pre-dispatch doubling ladder. Must be a positive integer no greater than `2147483647`.                                                                                                                                                                         | `30`       |
| `BATCH_JOB_RETRY_BACKOFF_MAX_SECONDS` | Maximum delay for queue retries and the per-item pre-dispatch doubling ladder. Must be a positive integer no greater than `2147483647` and at least `BATCH_JOB_RETRY_BACKOFF_SECONDS`.                                                                                                                           | `600`      |
| `BATCH_SETTLEMENT_ALLOWANCE_MS`       | Time held back from each worker job for the settlement checkpoint. Must be positive and less than `WORKER_JOB_TIMEOUT_SECONDS * 1000 - BATCH_MINIMUM_ITEM_COST_MS`; the allowance must leave more than one minimum item cost inside the job timeout, otherwise every job attempts a single item and re-enqueues. | `5000`     |
| `BATCH_MINIMUM_ITEM_COST_MS`          | Minimum expected cost of one batch item, used as the floor before an item runs and beneath the running average. Must be a positive integer.                                                                                                                                                                      | `2000`     |
| `BATCH_JOB_CONCURRENCY`               | Maximum number of batches issued in parallel by one worker process. Items inside each batch remain sequential. Must be a positive integer.                                                                                                                                                                       | `1`        |

Both roles validate `MAX_BATCH_ITEMS`, `BATCH_JOB_RETRY_LIMIT`, `BATCH_JOB_RETRY_BACKOFF_SECONDS` and `BATCH_JOB_RETRY_BACKOFF_MAX_SECONDS` before starting. The retry limit and both backoff settings must be positive integers no greater than `2147483647`, and the maximum backoff must be at least the base backoff. The worker role validates `BATCH_RETENTION_DAYS` and `BATCH_EXPIRY_SWEEP_MINUTES`, along with `BATCH_SETTLEMENT_ALLOWANCE_MS` and `BATCH_MINIMUM_ITEM_COST_MS`, before starting; the allowance must leave more than one minimum item cost inside the job timeout, otherwise every job attempts a single item and re-enqueues. The web role also validates `MAX_BATCH_REQUEST_BODY_BYTES` and refuses it when it is smaller than `MAX_REQUEST_BODY_BYTES`. `BATCH_JOB_CONCURRENCY` is read by the worker only, when it loads its configuration, and a value that is not a positive integer fails the worker before the queue starts.

The first batch job is enqueued by the web process and continuations are enqueued by the worker. Each queue send therefore uses the `WORKER_JOB_TIMEOUT_SECONDS` value in the process doing the send; configure that value identically in both roles when continuations must have the same budget as the first job.
...
The web process validates the shared credential-fetch settings and `CREDENTIAL_STATUS_DEFAULT_PURPOSES` after the request-body limit and before encryption validation or queue startup. The credential-fetch settings apply to verification, external registration and the supplier-source check used by re-verification. For scheme-document fetches behind seed URL entries, the seeded refresh and each per-scheme fetch of UNTP discovery, `FETCH_ALLOW_PRIVATE_URLS` permits private, loopback and reserved destinations, but it does not govern the discovery register fetch itself, the JSON-LD context loader or the schema loader. `FETCH_ALLOW_PRIVATE_URLS` also controls the existing stored-address URL checks on registrar, identifier-link, data-model, service and credential publishing routes. `CREDENTIAL_STATUS_DEFAULT_PURPOSES` controls the status purposes minted when issuance requests omit `statusPurposes`.
...
`CREDENTIAL_STATUS_DEFAULT_PURPOSES` is a comma-separated list of unique values from `revocation` and `suspension`; whitespace is trimmed and empty items are ignored. When it is unset or blank, the built-in default is `revocation`. An unsupported value, duplicate, or list with no purposes fails startup and names `CREDENTIAL_STATUS_DEFAULT_PURPOSES`, the configured value and the accepted values.

`CREDENTIAL_STATUS_LOCK_ACQUIRE_MS` sets the credential-issuance wait for the status-list transaction mutex, in milliseconds. When set, it must be a positive integer. The built-in default is 2,000 milliseconds, from `CREDENTIAL_STATUS_DEFAULT_LOCK_ACQUIRE_MS` in `packages/reference-implementation/src/lib/config/credential-status.config.ts`. If acquisition expires, issuance answers `503 STATUS_LIST_BUSY`. An invalid or non-positive value logs one warning naming the supplied value and the 2,000-millisecond default, then the mutex uses its own fallback.

The credential-fetch checks run at startup, but their readers are not cached, so a conflict introduced after startup is caught on the next read rather than at boot. Every route that reads a fetch setting then answers `500`, including the unauthenticated verify route, and the `error` field names the two conflicting variables and no values. A running process cannot be repaired by editing its environment: set the pair to a single name and restart the process, recreating the container where one is in use. `CREDENTIAL_STATUS_DEFAULT_PURPOSES` is also read for each issuance, so its configured default is not cached across requests.
...
Startup connects the job queue (a set of tables in the application database, managed by pg-boss) and creates the sending queues used by credential registration and batch submission. Creating them here rather than on the first request keeps each job insert to a single row written in the same database transaction as the credential or batch record, so acknowledged work always has its job and a job never runs for a record that rolled back. The web process only sends; the worker process runs the handlers. Startup fails if the database the queue needs cannot be reached.
...
The verification job also uses its remaining budget for a second provider call with the validity window skipped when the first call stops at that window, so proof and status can still be established.
...
For steady-state shutdown, health and non-Compose deployment guidance, see [Worker](./worker).

### Credential status operation settings

| Setting                                       | Default | Validation and effect                                                                                                                                                                                                                              |
| --------------------------------------------- | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `CREDENTIAL_STATUS_OPERATION_BUDGET_MS`       | `30000` | Positive decimal integer, at most `2147483647`. Bounds preliminary read, lock acquisition, set and read-back with one deadline. Reconcile and fresh status reads also use this budget.                                                             |
| `CREDENTIAL_STATUS_RECONCILE_GRACE_MS`        | `5000`  | Positive decimal integer, at most `2147483647`. Added to a pending deadline before reconciliation is admitted. It does not automatically clear intent.                                                                                             |
| `CREDENTIAL_STATUS_MUTATION_ENABLED`          | `false` | Exact `true` or `false`; unset or empty disables mutation. The lock identity changes at process start for issuance and status sets. The compatibility lock serialises mixed old and new processes, and a drain is still recommended before deploy. |
| `CREDENTIAL_STATUS_MULTIPLE_PURPOSES_ENABLED` | `false` | Exact `true` or `false`; unset or empty keeps issuance to one status purpose because UNTP v0.7.0 schemas accept one `credentialStatus` object. Set it to `true` to allow more than one purpose.                                                    |

Invalid non-empty settings fail web boot before requests are accepted. The two durations use their defaults when unset or blank. Both Compose definitions forward these values to the web service only. The worker does not perform issuer status mutations. `CREDENTIAL_STATUS_LOCK_ACQUIRE_MS` continues to bound lock acquisition within the operation's remaining allowance; status changes distinguish contention (`STATUS_LIST_BUSY`) from unavailable coordination capacity (`STATUS_COORDINATION_UNAVAILABLE`). See [Credential status recovery](./credential-status-recovery) for enablement, the locking limit and recovery after an uncertain outcome.

When `CREDENTIAL_STATUS_MULTIPLE_PURPOSES_ENABLED=false`, `CREDENTIAL_STATUS_DEFAULT_PURPOSES` must name no more than one purpose. The same rule applies to a request that supplies more than one `statusPurposes` entry. Web boot refuses a multi-purpose default until the setting is enabled.
