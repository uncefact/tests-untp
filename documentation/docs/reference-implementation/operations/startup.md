...
For the worker role, it validates `RI_HTTP_USER_AGENT`, `CACHE_MAX_ENTRIES`, `BUNDLED_ARTEFACTS_FALLBACK`, `MAX_BATCH_ITEMS`, `BATCH_JOB_RETRY_LIMIT`, `BATCH_JOB_RETRY_BACKOFF_SECONDS`, `BATCH_JOB_RETRY_BACKOFF_MAX_SECONDS`, and the required `DATA_ENCRYPTION_KEY`; it skips the web-only settings. As part of worker preflight, `resolveWorkerConfiguration` validates `WORKER_JOB_TIMEOUT_SECONDS`, `LIBRARY_RECONCILE_PENDING_RUNS_CRON`, `LIBRARY_RECONCILE_PENDING_RUNS_BATCH_SIZE`, `BATCH_RETENTION_DAYS`, `BATCH_EXPIRY_SWEEP_MINUTES`, `BATCH_SETTLEMENT_ALLOWANCE_MS`, `BATCH_MINIMUM_ITEM_COST_MS` and `BATCH_JOB_CONCURRENCY`.
