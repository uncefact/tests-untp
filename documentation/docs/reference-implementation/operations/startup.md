...
### Stored Copy Read Timeout

`LIBRARY_STORED_COPY_READ_TIMEOUT_MS` bounds one read of a durable copy back from this deployment's own storage service, end to end (default 10000, maximum 120000). Both processes read it. The web process uses it for the key-bearing recovery on `POST /api/v1/library/{id}/verify`, which reads the record's own copy inside the request; the worker uses it before a verification generation runs. Only the worker checks it at boot, as described under [Worker Boot](#worker-boot); the web process reads it when that route runs, so a value it cannot use fails the request rather than the startup. Give both containers the same value: the Compose files pass it to the web and worker services alike, and raising it for a slow worker also raises what a caller can wait on that route.
...
The job finds `PENDING` verification generations whose `lastEnqueuedAt` is older than the complete verification retry ladder, and generations with no `lastEnqueuedAt` at all whose own `requestedAt` is older than that same cutoff. The cutoff is rounded up to a policy bound of at least 30 minutes. It settles each matching generation as `FAILED` with `VERIFICATION_UNAVAILABLE` and `retryable: true`, with a message telling the caller to re-verify. It does not re-enqueue the original job. One tick takes at most `LIBRARY_RECONCILE_PENDING_RUNS_BATCH_SIZE` generations (default 500, maximum 10000), oldest first, so a large backlog is worked down over successive ticks rather than inside one job attempt; the worker validates that variable at boot the same way as the cadence.
...
5. The worker's settings are read and checked: the reconciliation cadence (`LIBRARY_RECONCILE_PENDING_RUNS_CRON`), the per-tick cap (`LIBRARY_RECONCILE_PENDING_RUNS_BATCH_SIZE`) and the budget for reading a durable copy back from storage (`LIBRARY_STORED_COPY_READ_TIMEOUT_MS`, default 10000, maximum 120000; the web process reads this one too, as [Stored Copy Read Timeout](#stored-copy-read-timeout) describes). A value that cannot be used fails the boot with `worker.configuration-invalid`, naming the variable, before the queue exists.
