...
The [Reference Implementation](/docs/reference-implementation/overview) is a multi-tenant application for issuing, storing, and verifying UNTP-compliant verifiable credentials — including Digital Product Passports (DPPs), Digital Conformity Credentials (DCCs), Digital Facility Records (DFRs), and more.
...
The Reference Implementation delegates to several [dependent services](/docs/reference-implementation/system-architecture) for credential signing, storage, and identifier resolution, and supports a pluggable [adapter architecture](/docs/reference-implementation/services/service-architecture) for integrating with different service providers.

## UNTP Playground

The [UNTP Playground](/docs/untp-playground) is a web application for validating UNTP credentials against the specification. Upload a credential and receive immediate feedback on whether it conforms — useful both as a development aid during implementation and as a demonstration tool.

## Test Suites

The [Test Suites](/docs/test-suites) provide structured conformance testing across multiple dimensions:
...
The [Semantic Interoperability tests](/docs/test-suites/semantic-interoperability) verify that the credentials produced by an implementation are semantically consistent with the UNTP specification. This allows implementors to be conformant with the core specification while allowing for extensions.
