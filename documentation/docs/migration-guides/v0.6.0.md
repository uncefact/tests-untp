...
Newly issued credentials carry `statusListIndex` as an integer to match the published UNTP v0.7.0 schemas. The Bitstring Status List specification requires a string in base 10, while capture canonicalises either wire shape to a decimal string for stored status entries; revert the issued value to a string when the UNTP schema is corrected upstream. Previously issued credentials keep their numeric index and remain readable. New issuance defaults to the existing `revocation` purpose, and callers may opt into `revocation`, `suspension`, or both through `statusPurposes`. The issuance default is now configurable with `DEFAULT_STATUS_PURPOSES`; leaving it unset keeps today's behaviour, while `none` alone issues without status entries. A capture warning does not prevent issuance. The new `status` facts and capture state are visible on native library detail records; external records return no issuer-owned status facts.
