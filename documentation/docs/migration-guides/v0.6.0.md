...
### Bulk credential issuance storage and worker settings

The reference implementation adds the `CredentialBatch` and `CredentialBatchItem` tables. Apply the unreleased Prisma migration before starting either role. Existing credentials are unchanged; the batch tables start empty. Five settings come with it. `MAX_BATCH_ITEMS` (default `500`) caps how many items one submission may carry. `MAX_BATCH_REQUEST_BODY_BYTES` (default `52428800`, 50 MiB) caps the raw body of one batch submission and must be at least `MAX_REQUEST_BODY_BYTES`; the web role validates it. Each item's serialised JSON remains subject to `MAX_REQUEST_BODY_BYTES`. `BATCH_RETENTION_DAYS` (default `30`) sets how long a settled batch keeps its items. `BATCH_EXPIRY_SWEEP_MINUTES` (default `60`) sets how often the worker deletes them. `BATCH_JOB_CONCURRENCY` (default `1`) sets how many batches one worker runs at once, and items within a batch stay sequential. `MAX_BATCH_ITEMS`, `BATCH_RETENTION_DAYS` and `BATCH_EXPIRY_SWEEP_MINUTES` are validated at boot in both roles, the batch body bound at web boot, and the concurrency setting when the worker loads its configuration.

The worker now declares `credentials.issue-batch` and schedules two sweeps: `credentials.reconcile-batches`, on the existing `LIBRARY_RECONCILE_PENDING_RUNS_CRON` cadence, and `credentials.expire-batches`, on the `BATCH_EXPIRY_SWEEP_MINUTES` cadence. The issuance queue processes each batch sequentially, checkpoints continuations transactionally, and marks interrupted in-flight items `OUTCOME_UNKNOWN` rather than issuing them again. Review the [batch issuance runbook](../reference-implementation/operations/batch-issuance) before operating a `NEEDS_ATTENTION` batch.

A second migration, `20260918120000_credential_batch_cancel`, adds the `CANCELLED` batch and item states, the `cancelledCount` counter and the `cancelRequestedAt` timestamp for the batch cancellation route. Every worker must be upgraded before that route is exposed, because an older worker does not check cancellation between items. The [Reference Implementation v0.6 guide](./ri-v0.6#batch-cancellation) gives the rollout order.

The maintenance commands `batch:inspect-item` and `batch:resolve-item` provide the audited recovery path for a `NEEDS_ATTENTION` batch. Inspection shows item identifiers and the request digest by default; `--disclose-request` is required to print the decrypted original request. Resolution requires the inspected batch version and either a same-tenant credential id or a non-empty evidence reference.

The worker now records a non-empty resolution reason and timestamp on the resolved item. A pre-dispatch item fault is requeued per item with a doubling delay starting at `BATCH_JOB_RETRY_BACKOFF_SECONDS` (default 30 seconds) and capped at `BATCH_JOB_RETRY_BACKOFF_MAX_SECONDS` (default 600 seconds); `BATCH_JOB_RETRY_LIMIT` (default 4) sets the total item attempts, so never-attempted items are not held behind it. The terminal item error is `ITEM_ATTEMPTS_EXHAUSTED` with the last projected message. Batch submission applies the single-issuance `credentialStatus` refusal before writing any item or queue job, and returns the indexed pointer `items[n].credentialPayload.credentialStatus` for that refusal.
...
- **Simplified conformity claim structure**: Simplified `issuingParty` in the `Standard` and `administeredBy` in `Regulation` by removing the nested `idScheme` and `type` structure.
...
##### 5. Conformity Claim structure changes

**Key Changes:**

- Simplified `issuingParty` in the `Standard` and `administeredBy` in `Regulation` by removing the nested `idScheme` and `type` structure.
...
```
packages/reference-implementation/src/templates/v0.6.0/digital_facility_record/template.hbs
```
...
```
packages/reference-implementation/src/templates/v0.6.0/digital_identity_anchor/template.hbs
```
...
```
packages/reference-implementation/src/templates/v0.6.0/digital_conformity_credential/template.hbs
```
...
- **Restructured credential subject**: The `credentialSubject` now uses a `ProductPassport` type with an embedded `product` object, replacing the direct `Product` reference, to better encapsulate claims and metadata. As part of this change, the following fields have been moved from the Product level to the ProductPassport level: granularityLevel, dueDiligenceDeclaration, materialsProvenance, circularityScorecard, emissionsScorecard, and traceabilityInformation.
...
The `credentialSubject` has been restructured to use a `ProductPassport` type, which encapsulates a `product` object and additional metadata like `granularityLevel` and claims. The id of the `ProductPassport` should generally match the id of the corresponding `Product`.

The following fields have been moved from the `Product` level to the `ProductPassport` level: `granularityLevel`, `dueDiligenceDeclaration`, `materialsProvenance`, `circularityScorecard`, `emissionsScorecard`, and `traceabilityInformation`.
...
- Simplified `issuingParty` in `emissionsScorecard`.`reportingStandard` by removing the `type` field.
...
**Example: issuingParty structure change (Reference Standard schema)**
...
**Example: issuingParty structure change (Reporting Standard schema)**
...
**Example: dimensions structure change (Reporting Standard schema)**
...
##### 7. Traceability Information Structure Changes

The data type for `traceabilityInformation` has been changed from a single object to a list of objects to support multiple entries.

**Example: traceabilityInformation structure change (Standard schema)**
...
```json
{
  "credentialSubject": {
    "traceabilityInformation": {
      "valueChainProcess": "Concentrating",
      "verifiedRatio": ...,
      "traceabilityEvent":
        {
          "linkURL": "http://localhost:3000/gs1/01/09349502000021/21/20001?linkType=gs1:traceability",
          "linkName": "Example Transformation Event",
          "linkType": "...",
          "hashDigest": "...",
          "hashMethod": "SHA-256",
...
```
packages/reference-implementation/src/templates/v0.6.0/digital_product_passport/template.hbs
```
...
Update the data structure for the JsonForm component to conform to the new TransactionEvent schema, reflecting the renamed sourceParty and destinationParty properties. This is applicable only to Transaction Events. Make sure that other Event types do not include the fields 'sourceParty' and 'destinationParty'.
...
```
packages/reference-implementation/src/templates/v0.6.0/digital_traceability_event/template.hbs
```
