...
| `traceId`       | The active OpenTelemetry trace id when the log is written inside a valid span; absent when no valid span is active                                                                                           |
| `spanId`        | The active OpenTelemetry span id when the log is written inside a valid span; absent when no valid span is active                                                                                            |
| `traceFlags`    | The active OpenTelemetry span's trace flags when the log is written inside a valid span; absent when no valid span is active                                                                                 |
...
The process service names used for traces are configured on the [OpenTelemetry observability page](./observability).
