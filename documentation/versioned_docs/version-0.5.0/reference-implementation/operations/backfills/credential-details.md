...
Once the cause is fixed, a record whose descriptive fields failed to extract is retried by setting its `detailsStatus` back to `EXTRACTION_PENDING` and re-running. See [Re-running](#re-running).
