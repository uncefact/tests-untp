...
The Prisma-managed Reference Implementation tables remain in the database's default `public` schema; the connection URLs its tooling constructs always target `public`. The job queue is separate: this application uses `pg-boss` `^12.29.0` without passing a schema option, so pg-boss 12.29.0 uses its default `pgboss` schema, creates it when the queue starts, and stores the `job`, `job_common`, `schedule`, `subscription`, `queue`, `version`, `bam`, `warning`, `queue_stats` and `job_dependency` tables, plus per-queue job partitions, there. The installed pg-boss plan does not define an `archive` table. The database account therefore needs `CREATE` on the database for that schema. The `public` and `pgboss` schemas coexist. To drop the queue, stop both processes and run `DROP SCHEMA pgboss CASCADE`.
...
For an upgrade that may change the schema incompatibly, back up the database and keep its encryption key with it, as described in the [release notes' back-up warning](https://github.com/uncefact/tests-untp/blob/next/packages/reference-implementation/RELEASE_NOTES.md). Stop the previous version's web and worker containers, start the new web container so it runs the migrations, then start the worker. Follow [Before you upgrade](../../migration-guides/ri-v0.5#before-you-upgrade) and [Rollback](../../migration-guides/ri-v0.5#rollback) for the complete procedure.
