...
Each bridge is composed from standalone functions that are paired together: a **builder**, an **extractor**, a **subject summary extractor**, and, for credential types that carry a conformity claim, a pair of **conformity claim extractors**.
...
- The **subject summary extractor** receives a credential subject and returns the `id` and display `name` a library row shows for it, read from wherever that version's UNTP schema puts them. What matters here is the schema rather than the builder above, because issuance reads the payload the caller submitted, which the builder may have had no part in producing. Most versions never set this: the generic default reads the subject's own top-level `id` and `name`, which is where the schema puts them unless a version's structure says otherwise. A version overrides it only when the schema nests the identifying fields elsewhere (DPP `0.6.x` and DFR `0.6.x` read them from a nested `product` or `facility` object) or names them differently (DIA `0.7.0` reads `registeredName` instead of `name`). It receives the subject as the credential carries it. Where that is an array (as of UNTP 0.7.0, a traceability event may carry several), the generic default describes the first element. A row has one subject to show, so the choice is between showing nothing and picking one, and the first is the deterministic pick. It is a display convenience rather than a claim that this subject is the principal one, since JSON-LD treats multiple values as an unordered set and the credential itself remains the record of the full set. A version that wants a different rule for several subjects overrides it.
