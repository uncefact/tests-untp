...
Before publishing, the tenant's registrar and identifier scheme namespaces must already be registered with the Identity Resolver. The seed registers these namespaces only for the system tenant; a tenant publishing with its own registrar or scheme must register the corresponding namespaces with the resolver first.
