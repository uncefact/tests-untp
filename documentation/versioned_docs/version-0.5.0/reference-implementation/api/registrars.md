...
Operations on this page that accept a request body are subject to the shared [request body size limit](../operations/api-pagination#request-body-size).
...
| `url`                  | string | Yes      | A valid public http(s) URL for the registrar's website. Rejected with a 400 if it is not a valid, public http(s) URL, or if it carries leading or trailing whitespace. Private and reserved addresses are allowed only when `FETCH_ALLOW_PRIVATE_URLS` is exact lowercase `true`; see [Credential Fetch Settings](../operations/startup#credential-fetch-settings) |
...
| `url`                  | A valid public http(s) URL for the registrar's website. Rejected with a 400 if provided and not a valid, public http(s) URL, if it carries leading or trailing whitespace, or if it is an explicit `null` (unlike `idrServiceInstanceId`, `url` cannot be cleared through this API). Private and reserved addresses are allowed only when `FETCH_ALLOW_PRIVATE_URLS` is exact lowercase `true`; see [Credential Fetch Settings](../operations/startup#credential-fetch-settings) |
