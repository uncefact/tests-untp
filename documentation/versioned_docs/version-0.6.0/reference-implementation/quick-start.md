...
All API endpoints require a Bearer token except [`POST /api/v1/credentials/verify`](./api/credentials#verify-a-credential), which is public. The Docker Compose configuration includes a pre-configured service account. See [Authentication](./authentication#obtaining-a-token) for how to obtain a token.
