...
`rotate:encryption-key` has no dry-run flag. Run [`audit:encryption`](./encryption-audit) before the rotation and use its report as the preflight evidence.
...
2. Remove `OUTGOING_DATA_ENCRYPTION_KEY` from `.env`, along with any leftover `SERVICE_ENCRYPTION_KEY`. Both must happen before the application starts: containers capture their environment when created, so a variable removed from `.env` afterwards lives on in the running container until it is recreated, and startup refuses to run while a leftover `SERVICE_ENCRYPTION_KEY` holds a value different from the new `DATA_ENCRYPTION_KEY`. (The rotation command itself reads its two variables directly, so the leftover does not stop the rotation.)
3. Start the application and the background worker (recreate both containers on a compose deployment, since each captured its environment when created) and confirm a credential read and a service resolution work. Retire the old key only per the [retention rule](./key-management#backups-pair-with-the-key): retained backups taken while it was active still open only under it.
