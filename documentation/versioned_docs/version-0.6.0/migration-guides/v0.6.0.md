...
A batch item is one ordinary credential request in an ordered batch. It uses the same request shape as single issuance and is issued through the same path. The Reference Implementation stores batch progress in the `CredentialBatch` and `CredentialBatchItem` tables. For the migration, worker requirement, settings and operator procedures, read the [Reference Implementation v0.6 guide](./ri-v0.6#batch-credential-issuance).
